...
| **REQ-V1-T3-01** ✅ | 场内地点感装饰 | 空地稀疏路径石/草/石/远雾/篱笆；确定性；中心作业区与 tilled/crop 不落；`hasFacilities` 才出篱笆 | dual-track；`paintWorldDecor` + unit |
| **REQ-V1-T4-01** ✅ | 农作四态可读 | 空/翻/播/浇视觉拉开（contrast/border、水洼 sheen、seedVisible/scale、harvestLift） | dual-track；`tileVisuals.ts` + unit |
| **REQ-V1-T5-01** ✅ | 主角色块在场 | 非纯黑剪影：`playerPresenceOverlay` under/over 条带 + 暖 tint + 半透明精灵 | dual-track；`characterPresence.ts` + unit |
| **REQ-V1-T6-01** ✅ | HUD 密度软收 | `#objective-rail` 一行主目标 + details 次要；canvas `compactBody`；信息只收不删 | dual-track；`formatJourneyGuideBody` + app-shell |
| **REQ-V1-L01** ✅ | journey-complete 无教学残留 | 完成后自由经营文案（cta `自由经营`）；停 day-1 教学 toast/对白 | dual-track；`isJourneyTeachingActive` + `isJourneyTeachingDialogueBeat` |
