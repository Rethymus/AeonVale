...
import {
  auditPublicWorktree,
  classifyPublicCandidateGroup,
  shouldFailPublicWorktreeAudit,
} from '../../tools/public-worktree-audit.mjs';

describe('公开工作区审查', () => {
  it('按公开树边界分类未提交路径', () => {
    const report = auditPublicWorktree([
      ' M README.md',
      ' M src/app/main.ts',
      ' M docs/00-DESIGN-BRIEF.md',
      '?? DESIGN-NOTES.md',
      ' D .omc/state/session.json',
      '?? .agents/state.json',
      '?? test-results/portfolio/home.png',
...
it('把公开候选路径归入发布审查桶', () => {
    const report = auditPublicWorktree([
      ...
      ' M index.html',
      ' M assets/logo/logo-full.png',
      ' M src/vite-env.d.ts',
      ' M src/app/main.ts',
      ' M src/render/renderer.ts',
      ' M src/io/assets.ts',
      ' M src/sim/farm/actions.ts',
      ' M src/content/registry.ts',
      ' M tests/browser/smoke.spec.ts',
      ' M tests/unit/farm.test.ts',
      ' M tests/integration/mvp-slice.int.test.ts',
      ' M tests/property/farm.property.test.ts',
      ' M tests/replay/schema.ts',
      ' M tools/content-lint.ts',
      ' M scripts/manual-check.ts',
      '',
    ].join('\0'));
