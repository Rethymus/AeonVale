...
import { createWorld, createSimContext, simulateDay, advanceDay, DEFAULT_BALANCE } from '@sim';
...
describe('INT-01: 完整日循环', () => {
  ...
  it('simulateDay 清空事件列表（每步瞬态）', () => {
    ...
    expect(state.events.length).toBeLessThanOrEqual(Math.max(prevLen, state.events.length) + 10);
  });
