...
expect(daySummaryMessage(2, events, reg, false, '当前目标：去山谷集市补几颗种子，把第二轮药材接上。')).toBe(
 '首轮出货结清｜出货结算：得灵石 ×19｜凡间青苔×2（8）〔订单热需+2〕、晾晒灵草×1（6） 等｜用途：灵石先补种子、炉料与备劫消耗。｜下一步：去山谷集市补几颗种子，把第二轮药材接上。',
 );
 });
...
it('在非出货结算的普通过夜里也保留下一步引导，避免首轮反馈散回泛化文案', () => {
 expect(daySummaryMessage(3, [], reg, false, '当前目标：第二轮药材动线已成立，继续照料新苗、卖余货，或再扩一小片田。')).toBe(
 '首轮农务闭环已成｜第 3 日｜下一步：第二轮药材动线已成立，继续照料新苗、卖余货，或再扩一小片田。',
 );
 });
...
expect(daySummaryMessage(3, shippingEvents, reg, false, '当前目标：今天先去集市看看，再决定补不补货。')).toBe(
 '出货结算：得灵石 ×8｜凡间青苔×2（8）｜下一步：今天先去集市看看，再决定补不补货。',
 );

expect(daySummaryMessage(3, [], reg, false, '当前目标：今天继续稳住节奏，有余力再做别的。')).toBe(
 '第 3 日｜下一步：今天继续稳住节奏，有余力再做别的。',
 );
 });

it('在运行时传入多行 onboarding help text 时，只消费目标首行并把操作行转成接力提示', () => {
 expect(daySummaryMessage(
 4,
 [],
 reg,
 false,
 '当前目标：去山谷集市补几颗种子，把第二轮药材接上。\n意义：补种把一次收获变成稳定经营，后续才有炼丹库存。\n操作：按 Shift+Tab 打开地点目录，选集市服务后确认补种。\n动线：先去山谷集市补种，再回农庄接上第二轮。',
 )).toBe(
 '第 4 日｜下一步：去山谷集市补几颗种子，把第二轮药材接上。｜接力：按 Shift+Tab 打开地点目录，选集市服务后确认补种。',
 );
 });
...
expect(daySummaryMessage(
 ...
 '首轮出货结清｜出货结算：得灵石 ×8｜凡间青苔×2（8）｜用途：灵石先补种子、炉料与备劫消耗。｜下一步：去山谷集市补几颗种子，把第二轮药材接上。｜接力：按 Shift+Tab 打开地点目录，选集市服务后确认补种。',
 );
 });

it('在日终摘要已包含下一步时，不再重复拼接额外的 onboarding 推进 toast', () => {
 expect(composeEndDayToastMessage(
 '第 4 日｜下一步：去山谷集市补几颗种子，把第二轮药材接上。｜接力：按 Shift+Tab 打开地点目录，选集市服务后确认补种。',
 '下一步：回到农庄，把新买到的种子立刻播回田里。｜面向已翻土地按 Z 补种。',
 )).toBe(
 '第 4 日｜下一步：去山谷集市补几颗种子，把第二轮药材接上。｜接力：按 Shift+Tab 打开地点目录，选集市服务后确认补种。',
 );
 });
...
expect(daySummaryMessage(4, events, reg, false)).toBe(
 '出货结算：得灵石 ×12｜凡间青苔×3（12）｜明日关注：回农庄看看哪块地该补种、浇水或顺手再收一轮。',
 );
 });
...
expect(daySummaryMessage(6, events, reg, false)).toBe(
 '1 株灵草成熟｜明日关注：先把成熟灵草收掉，再决定出货、加工还是留作后用。',
 );
 });
...
expect(daySummaryMessage(6, events, reg, false)).toBe(
 '今日已有灵草入袋｜明日关注：先巡一遍农庄，看看要不要立刻出货、补种，或把新收药材转去加工。',
 );
 });
...
expect(daySummaryPresentation(
 6,
 harvestEvents,
 reg,
 false,
 '当前目标：先去旧茶棚歇脚，把今日闲居节奏落下来。',
 ).assetId).toBe('loc.tea-shed');
...
it('在首轮播种或浇水阶段，日终总览入口与 onboarding 对象主语保持一致', () => {
 ...
 false,
 '当前目标：播下第一颗青苔种或露根草种。',
 ).assetId).toBe('icon.seed.mossling');
...
expect(daySummaryPresentation(
 ...
 false,
 '当前目标：给刚种下的幼苗浇一次水。',
 ).assetId).toBe('icon.item.water-pail');
 });
...
it('在日终下一步已明确指向节庆会场时，优先承接到节庆地点图', () => {
 ...
 reg,
 false,
 '当前目标：先去节日会场看看今日摊位，再决定要不要参与会场试礼。',
 ).assetId).toBe('loc.festival-ground');
...
expect(daySummaryPresentation(
 ...
 reg,
 false,
 '当前目标：先看晾晒架旁还有没有空位，把秋收药材及时转成稳定货。',
