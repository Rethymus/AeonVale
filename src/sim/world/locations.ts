...
export type LocationServiceCommand =
 | 'show-location-encounter'
 | 'show-tea-shed'
 | 'show-greenhouse'
 | 'show-festival'
 | 'browse-shop'
 | 'browse-festival-stall'
 | 'browse-trade'
 | 'show-commission'
 | 'explore-valley'
 | 'explore-ruin'
...
const PORTFOLIO_HIDDEN_DIRECTORY_LOCATIONS = new Set<LocationId>([
 'herb-plot',
 'creek-field',
 'drying-yard',
 'array-shed',
...
function encounterLines(state: GameState, locationId: LocationId, npcId: string, npcName: string, birthday: boolean): readonly string[] {
 ...
 return birthday
 ? [`${npcName}在人群里向你举杯。`, '今日是生辰，也是节日；凡人能记住这一天，便算有心。']
 : [`${npcName}站在节日会场边缘。`, '热闹不是灵修独有的东西，山谷里的人也要靠这些日子喘口气。'];
 }
...
export function getLocationServiceOptions(state: GameState, locationId: LocationId): LocationServiceOption[] {
 ...
 return location.services.filter((service) => locationServiceAvailability(state, locationId, service).open).map((service) => {
