...
import { createWorld, createSimContext, simulateDay, DEFAULT_BALANCE } from '@sim';
...
describe('PBT-08: nearDeathBonus 单峰 @ (0,10%]', () => {
  it('峰值区间 (0,10%] 返回全局最大值 nearDeathPeak', () => {
    ...
    fc.assert(
      fc.property(fc.float({ min: Math.fround(0.001), max: Math.fround(0.09), noNaN: true }), ratio => {
  ...
  it('峰值区间比 (10%,25%] 区间大', () => {
    ...
    fc.assert(
      fc.property(fc.float({ min: Math.fround(0.001), max: Math.fround(0.09), noNaN: true }), fc.float({ min: Math.fround(0.11), max: Math.fround(0.25), noNaN: true }), (peakRatio, midRatio) => {
  ...
  it('(10%,25%] 区间比 (25%,50%] 区间大', () => {
    fc.assert(
      fc.property(fc.float({ min: Math.fround(0.101), max: Math.fround(0.25), noNaN: true }), fc.float({ min: Math.fround(0.251), max: Math.fround(0.5), noNaN: true }), (lo, hi) => {
  ...
  it('(25%,50%] 区间比 >80% 区间大', () => {
    fc.assert(
      fc.property(fc.float({ min: Math.fround(0.251), max: Math.fround(0.5), noNaN: true }), fc.float({ min: Math.fround(0.801), max: Math.fround(1.0), noNaN: true }), (lo, hi) => {
        expect(nearDeathBonus(lo, P)).toBeGreaterThanOrEqual(nearDeathBonus(hi, P));
      })
    );
  });
  ...
  it('任意 ratio ∈ (0,1] → bonus 始终 > 0', () => {
    fc.assert(
      fc.property(fc.float({ min: Math.fround(0.001), max: Math.fround(1.0), noNaN: true }), ratio => {
...
describe('PBT-09: 突破率有界 successRate ∈ [0.05, 0.95]', () => {
    ...
    fc.assert(
      fc.property(
        ...
        (stage, poisonRatio, xSurplusRaw) => {
          ...
          const prepScore = 0.5;
          let successRate = P.breakthrough.successBase + P.breakthrough.successPrepBonus * prepScore + P.breakthrough.successXSurplus * xSurplus + P.breakthrough.successPoisonPenalty * poisonRatio;
  ...
  it('丹毒=0 时成功率 ≥ 丹毒满时成功率（惩罚为负权重）', () => {
    fc.assert(
      fc.property(fc.integer({ min: 1, max: 6 }), _stage => {
...
describe('PBT-10: 时间前进 — state.day 单调 +1', () => {
  it('simulateDay N 次后 state.day 恰好 +N', () => {
    fc.assert(
      fc.property(fc.integer({ min: 1, max: 50 }), fc.integer({ min: 1, max: 999 }), (days, seed) => {
  ...
  it('day 不跳变：相邻两次 simulateDay 之差恒为 1', () => {
    fc.assert(
      fc.property(fc.integer({ min: 2, max: 30 }), fc.integer({ min: 100, max: 9999 }), (days, seed) => {
        const state = createWorld({ seed, width: 4, height: 4, content: reg, params: P });
        const ctx = createSimContext(seed, reg, P);
