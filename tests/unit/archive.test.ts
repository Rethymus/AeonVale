...
import {
 applyAction,
 archiveDonationCount,
 archiveDonationFlag,
 archiveMilestoneFlag,
 claimArchiveMilestone,
 createSimContext,
 createWorld,
 DEFAULT_BALANCE,
 donateToArchive,
 getArchiveDonations,
 getArchiveMilestones,
 isArchiveDonationComplete,
 isArchiveMilestoneClaimed,
 nextArchiveDonation,
...
it('捐满全部 7 份典藏后可领取典藏大成里程碑（灵石×10 + 根基/意志）', () => {
 ...
 mutateItem(state.player, 'item.sealed-herb', 1);
 for (const id of [
 'archive.recipe-fragment-primer', 'archive.broken-talisman-anatomy', 'archive.array-core-proof',
 'archive.herbal-wine-vintage', 'archive.poultice-formula', 'archive.compost-manual', 'archive.sealed-herb-codex',
 ]) {
...
it('累计 6 份捐献后可领取六架归真里程碑（灵壤肥×2 + 根基/意志）', () => {
 ...
 for (const id of [
 'archive.recipe-fragment-primer', 'archive.broken-talisman-anatomy', 'archive.array-core-proof',
 'archive.herbal-wine-vintage', 'archive.poultice-formula', 'archive.compost-manual',
 ]) {
 expect(donateToArchive(state, id).ok).toBe(true);
 }
