...
function queuedQualityShippingEntryCount(state: GameState): number {
 return Object.values(state.qualityShippingBin).reduce((sum, batch) => (
 sum + Object.values(batch ?? {}).filter((count) => count > 0).length
 ), 0);
}
...
function carriedQualityEntryCount(state: GameState): number {
 return Object.values(state.player.qualityInventory).reduce((sum, batch) => (
 sum + Object.values(batch ?? {}).filter((count) => count > 0).length
 ), 0);
}

export function storageResultToast(
...
export function storageFailureToast(
 mode: 'deposit' | 'withdraw',
 choice: ResultChoice,
 content: ContentRegistry,
): string {
...
export function shippingResultToast(
 mode: 'normal' | 'quality',
 choice: ResultChoice,
 state: GameState,
 ctx: SimContext,
...
export function shippingFailureToast(
 choice: ResultChoice,
 content: ContentRegistry,
): string {
...
export function firstHarvestMilestoneToast(
 ...
 return guidance
 ? `首轮收获：${herbName} 已入手｜${purpose}｜${guidance}`
 : `首轮收获：${herbName} 已入手｜${purpose}`;
}
...
export function facilityJobStartToast(
 action: 'drying' | 'sealing' | 'furnace',
 input: ResultChoice | null,
...
export function facilityCollectResultToast(
 result: FacilityJobResult,
 state: GameState,
 content: ContentRegistry,
): string {
...
export function facilityCollectFailureToast(
 action: 'drying' | 'sealing' | 'furnace',
 result: FacilityCollectFailureResult,
): string {
 const facilityName = action === 'drying' ? '晾晒架' : action === 'sealing' ? '封藏柜' : '炼符炉';
 return `${facilityName}收取未成：${result.reason}`;
}

export function facilityStatusToast(
 action: 'drying' | 'sealing' | 'furnace',
 result: FacilityStatusResult,
): string {
...
export function facilityFailureToast(
 action: 'drying' | 'sealing' | 'furnace',
 result: FacilityFailureResult,
): string {
 const prefix = action === 'drying' ? '晾晒失败' : action === 'sealing' ? '封藏失败' : '熔炼失败';
 return `${prefix}：${result.reason}`;
}

export function brewMaterialFailureToast(
 result: BrewMaterialFailureResult,
 content: ContentRegistry,
): string {
...
export function guardBeastFeedResultToast(
 choice: ResultChoice,
 result: GuardBeastFeedResult,
 content: ContentRegistry,
): string {
 return `投喂巡守兽：${describeChoice(choice, content)}，精力${result.vigor ?? '?'}，羁绊${result.bond ?? 0}`;
}

export function pillUseToast(
 pillId: string,
 result: PillUseResult,
 content: ContentRegistry,
): string {
...
export function storageResultToastPresentation(
 mode: 'deposit' | 'withdraw',
 choice: ResultChoice,
 state: GameState,
 content: ContentRegistry,
...
export function storageFailureToastPresentation(
 mode: 'deposit' | 'withdraw',
 choice: ResultChoice,
 content: ContentRegistry,
): ToastPresentation {
...
export function shippingResultToastPresentation(
 mode: 'normal' | 'quality',
 choice: ResultChoice,
 state: GameState,
 ctx: SimContext,
...
export function shippingFailureToastPresentation(
 choice: ResultChoice,
 content: ContentRegistry,
): ToastPresentation {
 return {
...
export function firstHarvestMilestoneToastPresentation(
 events: readonly GameEvent[],
 content: ContentRegistry,
 nextStep: string,
...
export function firstShipmentMilestoneToastPresentation(
 baseToast: string,
 nextStep: string,
): ToastPresentation {
 return {
...
export function facilityJobStartToastPresentation(
 action: 'drying' | 'sealing' | 'furnace',
 input: ResultChoice | null,
 result: FacilityJobResult,
 content: ContentRegistry,
...
export function facilityCollectResultToastPresentation(
 result: FacilityJobResult,
 state: GameState,
 content: ContentRegistry,
): ToastPresentation {
...
export function facilityCollectFailureToastPresentation(
 action: 'drying' | 'sealing' | 'furnace',
 result: FacilityCollectFailureResult,
): ToastPresentation {
 return {
...
export function facilityStatusToastPresentation(
 action: 'drying' | 'sealing' | 'furnace',
 result: FacilityStatusResult,
): ToastPresentation {
 return {
...
export function facilityFailureToastPresentation(
 action: 'drying' | 'sealing' | 'furnace',
 result: FacilityFailureResult,
): ToastPresentation {
 return {
...
export function brewMaterialFailureToastPresentation(
 result: BrewMaterialFailureResult,
 content: ContentRegistry,
): ToastPresentation {
 return {
...
export function guardBeastFeedResultToastPresentation(
 choice: ResultChoice,
 result: GuardBeastFeedResult,
 content: ContentRegistry,
): ToastPresentation {
...
export function guardBeastFeedFailureToastPresentation(
 kind: 'no-guard-beast' | 'no-herb' | 'failed',
): ToastPresentation {
 return {
 message: kind === 'no-guard-beast'
 ? '尚无巡守兽'
 : kind === 'no-herb'
 ? '无可投喂灵草'
 : '投喂失败',
...
export function pillUseToastPresentation(
 pillId: string,
 result: PillUseResult,
 content: ContentRegistry,
): ToastPresentation {
