...
### 8.24 第二十轮（2026-09-11）：docs/31 迭代 3 首批（宽脉桥 + 劫式配方公示）

1. **宽脉桥（wide conductor）**：`BlockModifier` 扩 `'wide'`——beam 续脉规则升级：
   rift 格若有任一四邻格上的 wide 水阵石即续脉（一石跨两连格断口）。
   生成器 `installConductorBridge` 构造式附着：stage≥4 以修饰族概率改凿
   **2 连格 rift**（沿雷路方向）并给石附 wide；玩家推一次即续接双断口。
   首批三修饰（逆折镜/焚绝缘/宽脉桥）自此齐备。
2. **劫式配方公示**（Hades 图鉴式的 HUD 层）：新 `tribulationRecipeLabel`
   枚举板面修饰×灵草型，HUD 预备段新增「配方 逆折镜×宽脉桥×雷引草」标签——
   玩家首次可在局内直接读出本局劫式的组合构成（docs/31 §3.3 配方公示 P1）。

验证：tsc 零错；单测+property+replay **2672/2672**（新 3 例：宽石贴落任一断格
双续脉、基型石对照只续自身格、stage5 样本实际出现宽脉桥）；golden 夹具
`--init` 再授权（确定性自检通过）；构建/治理/content:lint 全绿；
浏览器回归 **26/26**。

**迭代 3 余项**：ghost 预览（推石预演虚线光路）、残卷门控经济（修饰池上限=
已解锁阵石类修饰节点数）、配方图鉴跨局收集（ash 链记录已遇组合）。

### 8.25 第二十一轮（2026-09-11）：docs/31 迭代 3 余项首批（ghost 预览 + 配方图鉴）

1. **ghost 预览**（docs/31 §2.3 P2，纯展示层零新算法）：玩家贴近可推阵石时，
   `ghostBeamForPushableStone` 以克隆板面 + applyMove + traceBeam 计算"推后"
   假想光路，虚线（boltViolet 50% 透明 + 6/6 dash）叠加绘制；**会被假想光路
   烧到的新鲜灵草以暗金描框高亮**——推石前即可读出代价。reduceFx 态不渲染。
   灵草新地形同步获得专属视觉：雷引草=暖金草形+雷芒、护脉草=环盾轮廓。
2. **配方图鉴跨局收集**（docs/31 §3.3，硬传承 app 层 meta）：`SokobanMeta` 扩
   `encounteredRecipes`（组合键 = archetype|修饰排序|灵草型排序，上限 64 条
   防 localStorage 膨胀）；**入局即记录**（enterTribulation 记录本局实际配方）；
   loadMeta 旧档迁移容错。备劫"下一劫与修行因果"面板（紫劫兆拓层）新增图鉴
   计数句：已录 N 味配方 / 尚无记录提示——未遇组合仍藏于天劫（防剧透只给
   计数不给清单）。

验证：tsc 零错；单测+property+replay **2674/2674**（新 2 例：ghost 素材面
自洽——applyMove 重追与显式 traceBeam 一致且推后光路≠原光路；配方键排序
拼接与去重幂等）；构建/治理/content:lint 全绿；浏览器回归 **35/35**。
...
**§8.31 补充：test:fast 验证与导出面审计（同日）**：`pnpm test:fast`
