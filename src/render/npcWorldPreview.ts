...
const queuedShipping = Object.values(state.shippingBin).some((count) => count > 0)
 || Object.values(state.qualityShippingBin).some((batch) => Object.values(batch ?? {}).some((count) => count > 0));
...
function locationServiceWorldState(
 state: GameState,
 locationId: LocationId,
): Pick<LocationWorldPreviewPlacement, 'serviceReady' | 'serviceDone' | 'taskReady' | 'serviceAssetId'> {
 ...
 serviceReady: false,
 serviceDone: false,
 taskReady: incidentTaskLocation === 'ruin-gate'
 || commissionTaskLocation === 'ruin-gate'
 || specialOrderTaskLocation === 'ruin-gate'
 || ruinGateTaskReady(state),
 };
 case 'spirit-vein':
 case 'creek-field':
 case 'herb-plot':
...
function queuedShippingCount(state: GameState): number {
 ...
 const quality = Object.values(state.qualityShippingBin).reduce(
 (total, batch) => total + Object.values(batch ?? {}).filter((count) => count > 0).length, 0,
 );
...
function placementForLocation(
 state: GameState,
 locationId: LocationId,
 groupIndex: number,
): Pick<NpcWorldPreviewPlacement, 'locationId' | 'x' | 'y'> {
...
function ambientNpcWorldPreviewPlacements(
 state: GameState,
 scheduledCounts: ReadonlyMap<LocationId, number>,
 scheduledAssetIds: ReadonlyMap<LocationId, ReadonlySet<string>>,
): NpcWorldPreviewPlacement[] {
 return getActiveLocationDirectory(state)
 .flatMap((location) => {
 const scheduledIds = scheduledAssetIds.get(location.id) ?? new Set<string>();
 const assetIds = (AMBIENT_LOCATION_NPC_ASSETS[location.id] ?? [])
 .filter((assetId) => !scheduledIds.has(assetId));
...
function ambientVisibleNpcCount(
 state: GameState,
 locationId: LocationId,
 scheduledAssetIds: ReadonlyMap<LocationId, ReadonlySet<string>>,
): number {
...
return [...scheduledPlacements, ...ambientNpcWorldPreviewPlacements(state, scheduledCounts, scheduledAssetIds)]
 .sort((a, b) => a.locationId.localeCompare(b.locationId) || a.x - b.x || a.y - b.y || a.assetId.localeCompare(b.assetId));
}
