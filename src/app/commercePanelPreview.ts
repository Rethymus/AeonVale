...
export function tradeToastPresentation(
 offer: TradeOffer,
 indexLabel: string,
 confirmHint: string,
 content: ContentRegistry,
): CommerceToastPresentation {
...
export function tradeResultToastPresentation(
 offer: TradeOffer,
 ...
 ? `交易成功：得 ${receiveName}×${offer.receive.qty}`
 : `交易失败：${reason ?? preview.title}`,
 assetId: outcome === 'success' ? preview.assetId : (preview.panelAssetId ?? preview.assetId),
 };
}

export function shopPanelPreview(
 kind: 'shop' | 'festival-stall',
 item: ShopItem | FestivalStallItem,
 content: ContentRegistry,
): CommercePanelPreview {
 return {
...
export function shopToastPresentation(
 kind: 'shop' | 'festival-stall',
 item: ShopItem | FestivalStallItem,
 indexLabel: string,
 confirmHint: string,
...
export function shopResultToastPresentation(
 kind: 'shop' | 'festival-stall',
 item: ShopItem | FestivalStallItem,
 outcome: 'success' | 'failure',
 content: ContentRegistry,
 ...
 return {
 message: outcome === 'success'
 ? `${action} ${item.displayName}×1`
 : `${failure}：${item.displayName}`,
 assetId: outcome === 'success' ? preview.assetId : (preview.panelAssetId ?? preview.assetId),
 };
}
