...
describe('天象因果链：灵气潮汐→妖兽潮 ', () => {
  ...
  it('完整因果：潮汐生长加成→灵草当日成熟→同日引兽', () => {
    ...
    simulateDay(
      ...
      {
        actions: [
          { kind: 'till', at: { x: loam.x, y: loam.y } },
          { kind: 'sow', at: { x: loam.x, y: loam.y }, seedId: 'seed.mossling' }
        ]
      },
      ctx
    );
    ...
    const evs = simulateDay(
      state,
      {
        actions: [
          { kind: 'water', at: { x: loam.x, y: loam.y } },
          { kind: 'channel-qi', at: { x: loam.x, y: loam.y } }
        ]
      },
      ctx
    );
