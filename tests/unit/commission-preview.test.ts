...
describe('commission preview', () => {
  ...
  it('builds commission toast presentations from preview details and asset', () => {
    ...
    expect(commissionToastPresentation(activeSpecialOrderPanelPreview(order, reg, '镇守事务', 1), '镇守告示', '空格/E/回车提交或领奖·Esc返回')).toEqual({
  ...
  it('keeps the preview title while flattening remaining lines for board toasts', () => {
    ...
    expect(commissionToastPresentation(mainlineQuestPanelPreview(quest, reg), '告示板', '空格/E/回车推进·Esc返回')).toEqual({
  ...
  it('reuses mainline and ruin preview art for claim result toasts', () => {
    ...
    expect(
      ruinChapterClaimToastPresentation(chapter, 7, reg, {
        title: '兽痕留书',
        floorStart: 11,
        floorEnd: 15
      })
