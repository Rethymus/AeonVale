...
const RAW_HERBS = [
  ...
  // —— M6 内容广度扩充（docs/18 §9.1）：补早期种田多样性 + 温性/避雷草缺口 ——
  {
    id: 'herb.stonegrain',
    displayName: '粟石草',
    tier: 1 as const,
...
const RAW_RECIPES: RecipeDef[] = [
  ...
  // —— M6 广度：新材料路径涌现（同效丹、不同料/火候），docs/06 非线性炼丹 ——
  {
...
export function buildRegistry(): ContentRegistry {
  ...
  const compatibleSchemaHashes = ['1eb5f343', 'c7b88545', '2d0d866a', '8650fece', 'e20ed1b', '467bd1a7']; // M0–M3 / M4（飞升丹前）/ M5（高tier丹药前/铁骨丹前/紫雷前兆前/广度扩充前=M5终态）：旧档可无损补默认字段继续读取。
