...
### 6.8 线上性能审计（已落地，2026-09-16）

Playwright/CDP 实测线上站点 Core Web Vitals + 天劫棋盘帧预算：唯一超
阈值项为冷载 LCP 4.0s（3.2MB 概念图 PNG 作标题背景）；WebP 转写后
2.0s 回到 good 区间，非JS载荷 -79%；CLS/TBT=0、棋盘帧预算 p95=16.7ms
60fps 零掉帧（无进一步优化项）。完整数据链见 docs/32 §24。

### 6.9 移动维度性能复测与字体预载（已落地，2026-09-16）

移动视口 + Slow 4G + 4x CPU 实测发现启动链串行缺陷（字体仅由 JS 发现，
LCP 中位 7.2s）；index.html preload 并行化后 LCP 5.4s、字体发现提前
3.7s，徽记降采样 103→31KB。字体 313KB 为文案语料约束（子集工具按 src
...
### 6.11 首个无人值守周度周期完成（2026-09-22 里程碑记录）

perf-vertical 首个 cron 周期于 09-21 完成，全程无人工介入：
平台 schedule 延迟 5.6 小时补投递（08:02 UTC success）→ 双 cron 互备
生效（后到触发被当日幂等守卫正确跳过，仅一轮重复采样且数据有效）→
feed 自动追加 3 条含修复后真实 TBT 的记录（19 条）→ 自动回提交入链 →
run summary 带判定双剖面全 ✓（CI runner 自动选 desktop-ci 带）。
同期移除孤儿夜间工作流 m5-nightly（阶段 3 退役遗留，脚本已删致每夜
失败——「github action 频繁报错」的根因，commit a977d04）。

验收细节见 docs/32 §24.7.2（含勘误更正：schedule 第 1 项由 ✗ 改判
✓ 延迟补投递）。监测体系自此进入纯无人值守运转。
