...
function diagnose(bot: BotPolicy, seeds: number, seedStart: number): void {
  ...
  const fatalAttempts = deaths
    .map((o) => o.tribulationAttempts.find((a) => !a.survived))
  ...
  console.log(`breakthroughs 分布（最高推进深度）:`, histogram(outcomes.map((o) => o.breakthroughs)).map(([k, v]) => `brk${k}=${v}`).join(' '));
  console.log(`stageReached 分布:`, histogram(outcomes.map((o) => o.stageReached)).map(([k, v]) => `s${k}=${v}`).join(' '));
  console.log(`deathCause 分布:`, histogram(outcomes.map((o) => o.deathCause ?? 'null').map((d) => d.replace('-death', ''))).map(([k, v]) => `${k}=${v}`).join(' '));
  console.log(`致命天劫 stage 分布:`, histogram(fatalAttempts.map((a) => a.stage)).map(([k, v]) => `s${k}=${v}`).join(' '));
