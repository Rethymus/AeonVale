...
function readyFacilityCountByKind(state: GameState, kinds: readonly FacilityKind[]): number {
 ...
 return Array.from(state.facilities.values()).filter((facility) => (
 kindSet.has(facility.kind) && (facility.job?.daysRemaining ?? 1) <= 0
 )).length;
}
...
function queuedShippingEntryCount(state: GameState): number {
 ...
 const quality = Object.values(state.qualityShippingBin).reduce((sum, batch) => (
...
function childFacilityActionSignalLine(
 prefix: string,
 state: GameState,
...
const readyOrder = specialOrderTaskLocationId(state) === 'valley-market'
 ? getActiveSpecialOrders(state).find((order) => order.remaining <= 0) ?? null
 : null;
...
const readyOrder = specialOrderTaskLocationId(state) === locationId
 ? getActiveSpecialOrders(state).find((order) => order.remaining <= 0) ?? null
 : null;
 if (readyOrder) segments.push(`订单可领：${readyOrder.title}`);
...
export function formatLocationActionSignalLine(
 state: GameState,
 ...
 case 'ruin-gate':
 return incidentActionSignalLine(prefix, state, locationId)
 ?? locationTaskSignalLine(prefix, state, locationId)
 ?? ruinGateActionSignalLine(prefix, state);
 case 'spirit-vein':
 case 'creek-field':
 case 'herb-plot':
 return incidentActionSignalLine(prefix, state, locationId)
