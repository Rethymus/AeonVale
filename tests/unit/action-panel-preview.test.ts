...
import {
 farmActionMenuPreview,
 farmActionMenuToastPresentation,
 npcActionMenuPreview,
 npcActionMenuToastPresentation,
 npcBrowsePanelPreview,
 npcBrowseToastPresentation,
 npcGiftPanelPreview,
 npcGiftResultToastPresentation,
 npcGiftToastPresentation,
 npcQuestPanelPreview,
 npcQuestResultToastPresentation,
 npcQuestToastPresentation,
 npcUnavailableToastPresentation,
} from '@app/actionPanelPreview';
...
expect(farmActionMenuPreview('facility-collect', state, ctx).details).toContain(
 '身旁暂无已建设施，可先去建造铺开经营位',
 );
...
expect(farmActionMenuPreview('storage-deposit', state, ctx).details).toBe(
 '仓流整理\n背包 2 格｜仓余 47 格\n先卸灵草材料，给采收、炼丹与布阵腾位',
 );
 expect(farmActionMenuPreview('shipping-normal', state, ctx).details).toBe(
 '日常回款\n可出 1 项｜已入箱 1 项\n回笼灵石，补种子、炉料与备劫消耗',
 );
 expect(farmActionMenuPreview('shipping-quality', state, ctx).details).toBe(
 '精品回款\n品质库存 1 项｜已入箱 1 项\n高品灵草优先换取更高阶修行资源',
 );
 expect(farmActionMenuPreview('storage-deposit', state, ctx).assetId).toBe('loc.farmstead');
 expect(farmActionMenuPreview('shipping-normal', state, ctx).assetId).toBe('loc.farmstead');
...
expect(npcGiftToastPresentation(
 npc,
 '灵石',
 true,
 '（2/3）',
...
expect(npcGiftResultToastPresentation(
 npc,
 'success',
 '灵石',
 true,
...
expect(npcQuestToastPresentation(
 npc!,
 quest,
 '（3/3）',
 '空格/E/回车推进·Esc返回',
...
it('falls back to npc portrait when quest reward has no icon mapping', () => {
 ...
 const preview = npcQuestPanelPreview(npc, {
