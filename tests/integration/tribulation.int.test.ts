...
describe('INT-03: 天劫端到端', () => {
  ...
  it('nearDeathBonus：(0,10%] HP 比 (50%,80%] HP 修为收益更高', () => {
    ...
    const bonusHigh = nearDeathBonus(0.6, DEFAULT_BALANCE);
  ...
  it('boltBaseDamage 随阶段线性递增', () => {
    ...
    expect(boltBaseDamage(1, P)).toBe((P.lightning.damage.base + P.lightning.damage.stageSlope * 1) * 1000);
  });
});
