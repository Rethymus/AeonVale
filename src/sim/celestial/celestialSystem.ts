 ...
 */
export function selectCelestialEvent(defs: readonly CelestialEventDef[], recentEventIds: readonly string[], rng: Rng): CelestialEventDef | null {
...
export function startSeasonalFestivalIfDue(state: GameState, ctx: SimContext): boolean {
  ...
  const due = [...ctx.content.events.values()].find(e => e.seasonal && e.seasonal.season === state.season && e.seasonal.day === state.seasonDay);
  if (!due) return false;
  state.activeEvent = {
    defId: due.id,
