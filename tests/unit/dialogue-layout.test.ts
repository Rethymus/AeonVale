...
import {
 ...
 DIALOGUE_LAYOUT_LIMITS,
 dialogueBoxLayout,
 dialogueTextLayoutStyle,
...
function expectLayoutContainsText(textHeight: number, hasPortrait: boolean): void {
 ...
 expect(layout.textY + textHeight).toBeLessThanOrEqual(
 layout.y + layout.height - DIALOGUE_LAYOUT_LIMITS.paddingY,
 );
 expect(layout.textX + style.wordWrapWidth).toBeLessThanOrEqual(
 layout.x + layout.width - DIALOGUE_LAYOUT_LIMITS.paddingX,
 );

 if (hasPortrait) {
 expect(layout.portraitY + layout.portraitSize).toBeLessThanOrEqual(
 layout.y + layout.height - DIALOGUE_LAYOUT_LIMITS.paddingY,
 );
 }
}
...
describe('dialogue layout', () => {
 ...
 const mixedLines = [
 '采药女·SeasonalCultivationRouteWithoutWhitespaceOrManualBreaks',
 '把灵草、spiritStoneReserveAndArrayCoreInventory、丹药与引劫准备接成同一条日常动线。',
 'NextStep:ReturnToTheFarmsteadAndKeepTheSecondHarvestMoving。',
 ];
 ...
 const maximumTextHeight = DIALOGUE_LAYOUT_LIMITS.bottom
 - DIALOGUE_LAYOUT_LIMITS.safeTop
 - DIALOGUE_LAYOUT_LIMITS.paddingY * 2;
