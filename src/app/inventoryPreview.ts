...
function candidateScore(candidate: PreviewCandidate, objectiveId: OnboardingObjectiveId | null): number {
 const qualityBonus = candidate.quality === 'treasure'
 ? 30
 : candidate.quality === 'spirit'
 ? 20
 : candidate.quality === 'mortal'
 ? 10
 : 0;
...
export function inventoryPreviewSelection(state: GameState, content: ContentRegistry): InventoryPreviewSelection | null {
 ...
 const candidate = collectCandidates(state)
 .sort((a, b) => candidateScore(b, objectiveId) - candidateScore(a, objectiveId)
 || itemSortRank(a.itemId) - itemSortRank(b.itemId)
 || a.itemId.localeCompare(b.itemId, 'zh-CN'))[0];
...
const title = content.items.get(candidate.itemId)?.displayName ?? candidate.itemId;
 ...
 const quantityLine = candidate.quality
 ? `${QUALITY_LABEL[candidate.quality]} × ${candidate.count}`
 : `数量 × ${candidate.count}`;
