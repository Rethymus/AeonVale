...
> **空灵根叙事解释**（`03` §2.2.1，D-34）：空灵根的"漏斗/漏勺"模型是已有机制的**叙事层面隐喻**，帮助理解各机制间的关系，非独立游戏系统。`bodyFoundation` = 净留存（天劫封堵漏洞后留存上升的等效表达）；`stage` 突破 = 天劫封堵部分漏洞；`nearDeathBonus` = 压力淬炼（漏勺在崩溃边缘被压实）。代码不需要吞吐量/漏失率/净留存三参数字段——bodyFoundation 的线性积累就是这个模型的简化实现。

> 命名约定：实现中的字段是 `bodyFoundation`（阶段体魄根基，见 `src/sim/world/player.ts`）；容量函数 `stageQiCap()` 只是 `bodyFoundationCap()` 的薄别名（`src/sim/progression/progression.ts`）。文档旧称"StageQi"一律指**阶段肉身淬炼进度**，不是灵修意义上的灵力，也没有对应的独立字段。玩家面 UI 可显示为"淬体进度"、"体魄"或"肉身境界"。
...
| 4 | 洗髓 | MarrowCleansing | 淬体增效丹、走火丹、引水阵（1级·自动浇水） | 紫雷为主 ×8–12 | 700 |
| 5 | 凝血 | BloodCondensing | 金雷出现、驯养守田兽、引水阵·扩脉（2级） | 紫+金雷 ×12–18 | 1100 |
| 6 | 雷骨 | ThunderBone | 诛仙雷、游方散仙神秘交易、引水阵·广泽（3级） | 金+诛仙雷 ×18–25 | 1600 |
...
- Stage 4–6：依次解锁高级丹药、阵法升级（**引水阵分级自动浇水 1/2/3 级**，对标星露谷洒水器，把日常浇水从日循环移除）、特殊事件（守田兽/交易）。
