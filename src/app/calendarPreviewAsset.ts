...
export function calendarSummaryPreviewAssetId(
 todayEntries: readonly CalendarEntry[],
 upcomingEntries: readonly CalendarEntry[],
): string {
 return calendarEntryPreviewAssetId(todayEntries[0] ?? upcomingEntries[0])
 ?? calendarEntryPreviewAssetId(upcomingEntries[0] ?? todayEntries[0])
 ?? locationPreviewAssetId('farmstead');
}
