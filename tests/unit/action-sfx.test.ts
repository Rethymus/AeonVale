...
describe('actionSfxQueue', () => {
  it('maps ruin delve and beast hunt damage events to hurt SFX', () => {
    expect(actionSfxQueue([event('ruin-delve', { damage: 8 }), event('beast-hunted', { damage: 4 })])).toEqual(['hurt']);
  });

  it('ignores non-damaging or unrelated action events', () => {
    expect(actionSfxQueue([event('ruin-delve', { damage: 0 }), event('beast-hunted', { damage: 0 }), event('beast-loot', { itemId: 'item.beast-core' }), event('explore', { grants: [{ itemId: 'item.recipe-fragment', count: 1 }] })])).toEqual([]);
  });
