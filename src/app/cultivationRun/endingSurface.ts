...
const ENDING_COPY: Readonly<Record<CultivationEndingKind, { readonly mark: string; readonly phase: string; readonly kicker: string; readonly caption: string }>> = {
  ...
  survived: { mark: '归', phase: '此世善终', kicker: '雷声已远 · 人间仍在', caption: '没有飞升的一世，也留下了完整的名字' },
...
export function createCultivationEndingSurface(options: CultivationEndingSurfaceOptions): CultivationStaticPhaseSurface {
  ...
  const frame = createCultivationInterludeFrame({
    ...
    extraStyles: ['[data-cultivation-interlude-host] .cr-cultivation-ending .cr-interlude__rail{border-inline-end:4px double rgb(var(--rgb-paperBorder) / .76);}', '[data-cultivation-interlude-host] .cr-cultivation-ending .cr-interlude__mark{border-radius:2px;box-shadow:inset 0 0 0 3px rgb(var(--rgb-shellPine) / .72);}', '[data-cultivation-interlude-host] .cr-cultivation-ending .cr-interlude__art{min-block-size:270px;}', '[data-cultivation-interlude-host] .cr-cultivation-ending .cr-interlude__art-image,[data-cultivation-interlude-host] .cr-cultivation-ending .cr-interlude__art-fallback{min-block-size:270px;}']
  });
  ...
  const statusText = onBeginAnotherLife ? '可以回到标题，也可以让另一位凡人接过未尽之路。' : '这一世已经完整收束，可以回到标题。';
  ...
  const anotherLifeButton = onBeginAnotherLife ? appendCultivationAction(frame.actions, '再开一世', status.id) : null;
