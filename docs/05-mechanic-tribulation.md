# 机制一：天劫倒计时 + 引雷承体阵法控伤 + 劫雷淬体

> 本文深挖《偷天换劫诀》引动天劫后的整套"雷场承劫 + 淬体突破"玩法，颗粒度到可直接实现。
> 上游宪法：`00-DESIGN-BRIEF.md` §3 核心循环、§5.1、§7 硬约束（C3 确定性 / C4 可无头测试 / C6 数据驱动）。
> 兄弟文档：种田即布阵的耦合见 `08-farming-system.md`；备劫丹药见 `06-mechanic-alchemy.md`；阶段曲线见 `09-progression-system.md`；数值收口见 `14-game-balance-and-math.md`。
...
- 但凡骨脆弱 → 必须布阵、种引雷草、服承雷丹，把雷势压到身体能承受的区间。
- 阵法分流太多 → 只吃余雷 → 淬体停滞；分流太少 → 一雷归西。

**"种田即布阵"**是本作最大的系统涌现卖点：你平时种灵草的布局，直接决定天劫时雷往哪劈、身体能吃到多少雷。这不是两套独立系统，而是同一张农田地图的双重语义（见 §6 与 `08-farming-system.md` §5）。
...
2. **秒级临战倒计时**：准备窗最后一日归零后，进入实时 `MM:SS` 压迫倒计时，随后硬切入雷场承劫。
...
3. **定格结束**：进入 **实时雷场承劫**——世界恢复 tick，第一道雷开始 targeting（见 §3）。此时玩家可移动、可嗑丹、可激活阵法，但**不可种田/翻地/炼丹/收获**（见 §8 限制）。

> **为什么不完全冻结到玩家准备好？** 因为天劫的张力来自"不可控"。若允许玩家无限准备，就退化成回合制解谜，失去肾上腺素。定格 1.5 s 只给"看清局面"的呼吸，不给"重新布阵"的余地——承雷准备必须在倒计时期间完成。
...
这是雷场承劫玩法的数学核心。**必须确定性可种子化**（C3），无头测试时同一 seed + 同一棋盘状态必须产生完全相同的落雷序列。
...
**(b) `MetalAttraction(t)` —— 金属性灵草导雷吸引倍率**
...
- `【可调参数】RodRadius`（默认 3 格）—— 金属性灵草的导雷吸引半径（曼哈顿距离）。
- `【可调参数】MetalPowerBase`（默认 2.0）—— 一株成熟金属性灵草的吸引力（成长度按比例缩放）。

> 这是"金属性灵草作引雷针"的物理化建模——它把雷往自己身上引，再由阵势把余雷导回玩家，形成可控淬体窗口。
...
如果所有 `W(t)` 都被绝缘阵压到 0（玩家完美布阵——几乎不可能），则 fallback：劈向最近的阵法边界格（"阵法被击穿"），并对该阵法造成结构损伤。
...
| **RodHit（引雷草/引雷阵代接）** | `chosen` 是金属性草或引雷阵阵心，玩家不在范围内 | 0 | **少量**（见 4.2） |
...
- `WardPillReduction` —— 来自 `承雷丹` buff（0–0.6，见 `06-mechanic-alchemy.md`）。
...
- 完全没阵法、没金属性草：雷主要按 `Conductivity × PlayerProximity × EpicenterBias` 劈，大概率直劈玩家或农田中心 → 高伤害高淬体。教学关会强制玩家在倒计时内至少放一个引雷草。
...
- **金属性灵草 = 活体引雷针**：平时是产出金属属性药性的炼丹材料，天劫时是吸引雷的引雷针。玩家种它的位置 = 阵法布局。
- **核心药草区 vs 外围引雷草区**：鼓励空间策略——把稀有/脆弱的核心药草（寒/热/温属性）种在中心（由绝缘阵分流稳住），把金属性引雷草种在外围（吸引雷远离中心）。
- **药性需求 vs 承雷需求的冲突**：如果当前丹方需要大量金属性草，你就得多种；但种太多引雷草会把雷全引到外围，身体只吃余雷，淬体不足。反之亦然。这是涌现的策略空间。
...
- [Plants vs. Zombies Wiki - Target Zombie（目标选择参考）](https://plantsvszombies.fandom.com/wiki/Target_Zombie)
- [Stanford MCTS Applied to PvZ（波次决策建模参考）](https://web.stanford.edu/class/aa228/reports/2020/final66.pdf)
...
- [Kingdom Defense / Tower Defense targeting patterns（通用目标选择参考）](https://gamedev.stackexchange.com/questions/9035/code-structure-level-design-plants-vs-zombies-game-level-dissection)
