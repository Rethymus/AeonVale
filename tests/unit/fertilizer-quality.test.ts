...
function tillableAt(state: GameState): { x: number; y: number } {
  const tile = state.tiles.find(candidate => !candidate.tilled && candidate.blockType === 'none' && candidate.soilType !== 'water' && candidate.soilType !== 'rock' && candidate.soilType !== 'metal-ore');
