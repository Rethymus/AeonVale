/**
 * 紫雷前兆 event.purple-omen 强制触发（docs/15 §4 / docs/07 / M5）。
 * stage4 修为恰满 → 跳过随机门强制触发，7 日倒计时，flag 防重；forced 事件不入随机抽样池。
 */
...
const STAGE4_CAP = DEFAULT_BALANCE.breakthrough.xCap[3] ?? 700_000; // stage4 修为上限（毫点，docs/14 §8.1）
...
describe('紫雷前兆 event.purple-omen 强制触发（docs/15 §4 / M5）', () => {
  it('stage4 修为恰满 → 强制触发（跳过随机门），置 flag', () => {
    const { state, ctx } = setup();
    state.player.stage = 4 as 4;
    state.player.cultivation = STAGE4_CAP; // 精确上限
  ...
  it('有激活事件时 forced 路径被跳过（不重复触发，仅倒计时）', () => {
    const { state, ctx } = setup();
    state.player.stage = 4 as 4;
    state.player.cultivation = STAGE4_CAP;
  ...
  it('stage5 不触发（即便修为达到 stage4 cap）', () => {
    const { state, ctx } = setup();
  ...
  it('stage<4 不触发（即便修为远超）', () => {
    const { state, ctx } = setup();
    state.player.stage = 3 as 3;
    state.player.cultivation = 999_000_000;
    tickCelestial(state, ctx);
    expect(state.flags.has('purple-omen-fired')).toBe(false);
    expect(state.activeEvent?.defId).not.toBe('event.purple-omen');
  });

  it('stage4 修为超过 cap 仍触发（阈值为 ≥ cap）', () => {
    const { state, ctx } = setup();
    state.player.stage = 4 as 4;
    state.player.cultivation = STAGE4_CAP + 1;
    tickCelestial(state, ctx);
    expect(state.flags.has('purple-omen-fired')).toBe(true);
    expect(state.activeEvent?.defId).toBe('event.purple-omen');
    expect(state.activeEvent?.daysLeft).toBe(7);
  ...
  it('stage4 但修为未满 cap 不触发', () => {
    const { state, ctx } = setup();
    state.player.stage = 4 as 4;
    state.player.cultivation = STAGE4_CAP - 1; // 差一点
    tickCelestial(state, ctx);
    expect(state.flags.has('purple-omen-fired')).toBe(false);
    expect(state.activeEvent?.defId).not.toBe('event.purple-omen');
  });
