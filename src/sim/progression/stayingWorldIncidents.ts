...
return {
 beastId: best.id,
 vigorCost: 1,
 pressureReliefBonus: guardBeastMasteryReady(best)
 ? 3 * MILLI
 : best.specialty === 'courier' || guardBeastSpecialtyReady(best) ? 2 * MILLI : MILLI,
 ...
 let best:
 | { beast: GameState['guardBeasts'][number]; tileId: number }
 | null = null;
...
return {
 ...
 vigorCost: 1,
 pressureReliefBonus: guardBeastMasteryReady(best.beast)
 ? 4 * MILLI
 : best.beast.specialty === 'array-warden' || guardBeastSpecialtyReady(best.beast) ? 3 * MILLI : 2 * MILLI,
...
return {
 ...
 pressureReliefBonus: guardBeastMasteryReady(best)
 ? 5 * MILLI
 : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI,
 mastery: guardBeastMasteryReady(best) || undefined,
 spentCountOverride: 0,
 specialtyProgress: 'field-ward',
 };
 }
 if (incident.id === 'incident.warden-commission') {
 // 巡守委托：有精力的巡逻巡守兽可带队夜巡，省下一份灵石酬金并额外减压。
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return {
 beastId: best.id,
 ...
 if (incident.id === 'incident.frost-blight') {
 // 霜害侵田：巡逻巡守兽可连夜护田，省下一份露根草并额外减压。
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return {
 beastId: best.id,
 ...
 if (incident.id === 'incident.wind-erosion') {
 // 风蚀灵田：巡逻巡守兽可挡风护苗，省下一份朝阳菇并额外减压。
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return {
 beastId: best.id,
 ...
 if (incident.id === 'incident.miasma-seep') {
 // 瘴气渗田：巡逻巡守兽可巡田驱瘴，省下一份和合叶并额外减压。
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return {
 beastId: best.id,
 ...
 if (incident.id === 'incident.spirit-stone-tribute') {
 // 灵石献祭：巡逻巡守兽可代为押运，省下一份灵石并额外减压。
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return {
 beastId: best.id,
 ...
 if (incident.id === 'incident.merchant-caravan') {
 // 商队过境：巡逻巡守兽可代为押货，省下一份晾晒灵草并额外减压。
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return {
 beastId: best.id,
 ...
 if (incident.id === 'incident.formation-collapse') {
 // 阵势崩塌：巡逻巡守兽可清场固基，省下一份破损法宝并额外减压。
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return {
 beastId: best.id,
 ...
 if (incident.id === 'incident.hermit-visit') {
 // 隐士造访：巡逻巡守兽可引路护行，省下一份残卷并额外减压。
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return {
 beastId: best.id,
 ...
 if (incident.id === 'incident.beast-core-bounty') {
 // 内丹悬赏：巡逻巡守兽可猎妖代缴，省下一份内丹并额外减压。
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return {
 beastId: best.id,
 ...
 if (incident.id === 'incident.mistfern-ritual') {
 // 雾蕨祭礼：巡逻巡守兽可入山采办，省下一份雾蕨并额外减压。
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return {
 beastId: best.id,
 ...
 if (incident.id === 'incident.frostmarrow-ward') {
 // 寒髓镇邪：巡逻巡守兽可守夜驱邪，省下一份寒髓莲并额外减压。
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return {
 beastId: best.id,
 ...
 if (incident.id === 'incident.compost-offering') {
 // 灵壤供奉：巡逻巡守兽可押运培土，省下一份灵壤肥并额外减压。
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return {
 beastId: best.id,
 ...
 if (incident.id === 'incident.array-core-tribute') {
 // 阵核供奉：巡逻巡守兽可押运镇守，省下一份阵核并额外减压。
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return {
 beastId: best.id,
 ...
 if (incident.id === 'incident.sealed-tribute') {
 // 封藏供奉：巡逻巡守兽可押运入库，省下一份封藏灵草并额外减压。
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return {
 beastId: best.id,
 ...
 if (incident.id === 'incident.mossling-cleanup') {
 // 青苔清理：巡逻巡守兽可翻地清场，省下一份青苔并额外减压。
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return {
 beastId: best.id,
 ...
 if (incident.id === 'incident.wine-offering') {
 // 药酒供奉：巡逻巡守兽可押运奠酒，省下一份药酒并额外减压。
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return {
 beastId: best.id,
 ...
 if (incident.id === 'incident.poultice-stockpile') {
 // 药膏备储：巡逻巡守兽可押运入库，省下一份药膏并额外减压。
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return {
 beastId: best.id,
 ...
 if (incident.id === 'incident.dried-herb-stockpile') {
 // 晾晒灵草备储：巡逻巡守兽可晒场翻理，省下一份晾晒灵草并额外减压。
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return {
 beastId: best.id,
 ...
 if (incident.id === 'incident.guard-relief') {
 // 守军劳军：巡逻巡守兽可代为押饷，省下一份灵石并额外减压。
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return {
 beastId: best.id,
 ...
 if (incident.id === 'incident.beast-pelt-tribute') {
 // 兽皮供奉：巡逻巡守兽可猎妖代缴，省下一份内丹并额外减压。
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return {
 beastId: best.id,
 ...
 if (incident.id === 'incident.stonegrain-relief') {
 // 粟石草济荒：巡逻巡守兽可押粮放赈，省下一份粟石草并额外减压。
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return {
 beastId: best.id,
 ...
 if (incident.id === 'incident.broken-talisman-tribute') {
 // 法宝碎件供奉：巡逻巡守兽可拆解代缴，省下一份碎件并额外减压。
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return {
 beastId: best.id,
 ...
 if (incident.id === 'incident.mistfern-feast') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 ...
 if (incident.id === 'incident.compost-field-rite') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.dewroot-tonic') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.balmleaf-tonic') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.suncap-feast') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.stonegrain-feast') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.recipe-archive') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.array-core-stockpile') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.sealed-archive') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.wine-archive') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.poultice-archive') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.spirit-stone-festival') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.talisman-forge') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.beast-core-forge') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.array-core-forge') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.compost-forge') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.sealed-forge') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.dried-herb-forge') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.wine-forge') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.poultice-forge') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.recipe-archive-bulk') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.spirit-stone-tribute-bulk') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.beast-core-bulk') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.talisman-bulk') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.mossling-bulk') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.sealed-bulk') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.dewroot-bulk') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.suncap-bulk') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.stonegrain-bulk') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.balmleaf-bulk') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.recipe-bulk') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.wine-bulk') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.poultice-bulk') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.dried-herb-bulk') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.array-core-bulk') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.compost-bulk') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.spirit-stone-grand') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.beast-core-grand') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.talisman-grand') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.sealed-grand') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.array-core-grand') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.compost-grand') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.dried-herb-grand') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.wine-grand') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.poultice-grand') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.recipe-grand') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.mossling-grand') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.dewroot-grand') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.suncap-grand') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.stonegrain-grand') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.balmleaf-grand') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.mistfern-grand') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.frostmarrow-grand') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.spirit-stone-centennial') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.beast-core-centennial') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.talisman-centennial') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 if (incident.id === 'incident.sealed-centennial') {
 const best = preferredGuardBeastForPatrol(state);
 if (!best) return null;
 return { beastId: best.id, vigorCost: 1, pressureReliefBonus: guardBeastMasteryReady(best) ? 5 * MILLI : best.specialty === 'field-ward' || guardBeastSpecialtyReady(best) ? 4 * MILLI : 3 * MILLI, mastery: guardBeastMasteryReady(best) || undefined, spentCountOverride: Math.max(0, incident.count - 1), specialtyProgress: 'field-ward' };
 }
 ...
 if (incident.id !== 'incident.beast-trace') return null;
