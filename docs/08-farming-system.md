...
# 种田系统：灵气驱动的生长 + 土壤导电性 + 空间布阵

> 本文设计种田核心流程、灵气生长模型、土壤系统、季节历法、地形导电性（与天雷/阵法耦合），以及"种田即布阵"的空间策略。
...
种田是**核心循环的入口之一**。它承担三重身份：

1. **资源产地**：灵草是炼丹唯一主料来源。
2. **承雷布局**（本作最大创新）：灵草种植位置 = 天劫时阵法布局。种田不是孤立的治愈玩法，而是**平时布阵**。
3. **慢节奏锚点**：90% 慢节奏由种田/炼丹/基础苦练共同构成（对齐 `00` §2.4 支柱 4）；种田是慢节奏的入口之一与灵气吞吐来源，非单一主轴，反衬天劫的失控。

灵感坐标：星露谷物语（翻地/浇水/季节/收获循环）× 修仙灵气设定 × 雷场承劫前置布局。
...
- 金属性草对灵气需求低（×1.2 通用，可在贫瘠地种）—— 这正是它适合做外围引雷草的原因。
...
不同土壤类型有不同 `Conductivity`，直接服务于天雷 targeting（见 `05-...md` §3.2）。这是种田与雷场承劫的耦合点：

| 土壤类型 | 来源 | Conductivity | 肥力上限 | 备注 |
|---------|------|-------------|---------|------|
| 湿润泥土 WetLoam | 浇水后/水边 | 1.8（强导电） | 80 | 高肥但易引雷 |
| 普通农田 Loam | 标准翻地 | 1.0（基准） | 100 | 默认 |
| 干燥沙土 DrySand | 干旱地/沙地 | 0.5（弱导电） | 50 | 低肥但承雷少 |
| 焦土 Scorched | 雷击/魔修过境 | 1.2 | 30 | 需翻新才能复种 |
| 绝缘垫层 Insulated | 玩家铺设（消耗材料） | 0.1 | 70 | 主动绝缘，分流核心雷势 |
| 金属矿露头 MetalOre | 矿脉地 | 1.5 | 0（不可种） | 强引雷，影响邻格 |
| 岩石 Rocky | 山岩 | 0.3 | 0 | 不可种 |

> 设计：玩家可主动改造土壤（铺绝缘垫、引水变湿泥土），但每种改造有 trade-off。例如把核心药草区铺成绝缘垫层分流雷势，但肥力上限降到 70，需更频繁施肥。这就是"种田决策 = 承雷路径决策"。
...
│  │ [玩家家]              │      │
│  └──────────────────────┘      │
│  [水域]  [引雷草]                │  ← 水域天然引雷，外侧布置引雷草
└────────────────────────────────┘
```

**空间策略原则**：
- **核心药草区**：稀有/脆弱/高产灵草，种在中心，由绝缘阵分流稳住，土壤铺绝缘垫层（牺牲肥力换安全），高灵气带。
- **外围引雷草区**：金属性草环绕，种在外围/金属矿旁/水域边（天然高导电处），把雷心锁在外圈，并把余势导入可承受的淬体窗口。
- **诱饵区**：便宜快熟的凡间作物或低级灵草，故意种在引雷阵心，给雷劈，换取核心区稳定与少量劫灰收益。
...
- 水域天然高导电 → 玩家可在水域旁种引雷草"双重吸引"。

**这就是"种田即布阵"**：玩家平时的每一锄头、每一粒种子的位置，都在为天劫布阵。无需切换到"布阵模式"——种田的布局就是防御的布局。
...
| 外围引雷草区 | 外环 | 普通或金属矿邻 | 金属性草 ×多 | 引雷阵阵眼 | 中低（金草耐受） |
| 诱饵/缓冲区 | 引雷阵心 | 普通 | 廉价凡间作物 | — | 低 |

### 6.2 布局博弈

布局不是一劳永逸：
- **阶段升级**：高阶天劫需要更强引雷草（紫雷需更多金草），核心区可能需要扩建绝缘阵。
- **天象扰动**：`灵脉异动` 会改变灵气分布，可能迫使玩家迁移核心区。
- **季节调整**：冬种寒草要移到核心（导雷压力小），夏秋雷多发要加强外围。
...
- 代价：消耗资源，且聚灵会**降低外围 QiDensity**（影响引雷草生长，但金草耐受低灵气，故可承受）。
...
- **Q4**：土壤导电性的改造深度——是否允许玩家任意铺设绝缘垫？这影响"种田即布阵"的灵活度。本文允许但限制材料成本。
