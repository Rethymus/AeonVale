...
describe('location preview helper', () => {
  ...
  it('resolves visible npc names into preview npc ids in order', () => {
    ...
    expect(locationPreviewNpcIds(location({ npcs: ['游方散修', '采药女', '不存在'] }), npcNameToId)).toEqual(['npc.wandering-cultivator', 'npc.herb-gatherer']);
  });
  ...
  it('makes the farmstead focus reason follow the same actionable logistics pressure as other P0 surfaces', () => {
    ...
    expect(locationPreviewFocusReason(state, null, 'farmstead', 'show-farm-work', 0)).toBe('昨夜回款还挂在箱里，先回农庄把出货结清。');
    ...
    expect(locationPreviewFocusReason(state, null, 'farmstead', 'show-farm-work', 0)).toBe('仓储已经顶满，先回去清仓再安排采收与外出。');
  });
  ...
  it('makes the farmstead focus reason follow field-level urgency before generic farm entry copy', () => {
    ...
    expect(locationPreviewFocusReason(state, null, 'farmstead', 'show-farm-work', 0)).toBe('有 1 块灵田正缺水，先回农庄把当日水路补稳。');
    ...
    expect(locationPreviewFocusReason(state, null, 'farmstead', 'show-farm-work', 0)).toBe('有 1 块灵田灵气偏弱，回去补灵后生长才稳。');
    ...
    expect(locationPreviewFocusReason(state, null, 'farmstead', 'show-farm-work', 0)).toBe('昨夜还留着 1 块焦土地，先回去把田面翻新。');
  });
  ...
  it('switches farmstead root preview summaries onto the array-shed thread when breakthrough is ready', () => {
    ...
    const ctx = locationPreviewSummaryContext(state, location({ id: 'farmstead', displayName: '农庄', description: '主角以凡骨苦练、种灵草与炼体的据点。' }), [service({ locationId: 'farmstead', service: 'farm-work', label: '耕作', command: 'show-farm-work', commandLabel: '查看农事' })], service({ locationId: 'farmstead', service: 'farm-work', label: '耕作', command: 'show-farm-work', commandLabel: '查看农事' }), []);
  ...
  it('uses stateful why-now guidance for tea shed entries instead of generic flavor text', () => {
    ...
    expect(locationPreviewFocusReason(state, null, 'tea-shed', 'show-tea-shed', 0)).toBe('留世后可来这里歇脚听闻，先记住这处日常落点。');

    state.postAscension.mode = 'stayed-in-world';
    expect(locationPreviewFocusReason(state, null, 'tea-shed', 'show-tea-shed', 0)).toBe('先来歇脚回气，把今日传闻和人情一起收下。');

    state.flags.add(`tea-shed-visit.${state.day}`);
    expect(locationPreviewFocusReason(state, null, 'tea-shed', 'show-tea-shed', 0)).toBe('今日茶棚已歇过脚，先把传闻记下，再转去别处推进。');
  });
  ...
  it('uses stateful why-now guidance for greenhouse entries based on unlock and climate pressure', () => {
    ...
    expect(locationPreviewFocusReason(state, null, 'greenhouse', 'show-greenhouse', 0)).toBe('留世后可来这里养护育苗，先把这条后续经营线记住。');

    state.postAscension.mode = 'stayed-in-world';
    state.stayingWorld.greenhouseClimate = 32_000;
    expect(locationPreviewFocusReason(state, null, 'greenhouse', 'show-greenhouse', 0)).toBe('棚势偏弱，今天先来回暖，免得离季苗势继续塌。');

    state.stayingWorld.greenhouseClimate = 68_000;
    expect(locationPreviewFocusReason(state, null, 'greenhouse', 'show-greenhouse', 0)).toBe('先巡暖棚，把育苗与回养节奏稳在今天这轮。');

    state.flags.add(`greenhouse-tended.${state.day}`);
    expect(locationPreviewFocusReason(state, null, 'greenhouse', 'show-greenhouse', 0)).toBe('今日暖棚已养护过，先把棚里成果接走，再回主线农务。');
  });

  it('carries high-priority action signals into selection toasts for the same location thread', () => {
    expect(locationSelectionToastPresentation('服务', location({ id: 'farmstead', displayName: '农庄', description: '主角以凡骨苦练、种灵草与炼体的据点。' }), service({ locationId: 'farmstead', service: 'farm-work', label: '耕作', command: 'show-farm-work', commandLabel: '查看农事' }), '空格/E/回车执行·Esc返回', '翻地、补种、浇水、收获与出货都从这里收口。', '要务：待收设施 1 座｜出货箱待结 2 项', 'facility.shipping-bin')).toEqual({
  ...
  it('allows farmstead root service toasts to reuse the current farmstead focus asset thread', () => {
    expect(locationSelectionToastPresentation('服务', location({ id: 'farmstead', displayName: '农庄' }), service({ locationId: 'farmstead', service: 'farm-work', label: '耕作', command: 'show-farm-work', commandLabel: '查看农事' }), '空格/E/回车执行·Esc返回', '昨夜回款还挂在箱里，先回农庄把出货结清。', '要务：出货箱待结 1 项', 'facility.shipping-bin').assetId).toBe('loc.farmstead');

    expect(locationSelectionToastPresentation('服务', location({ id: 'farmstead', displayName: '农庄' }), service({ locationId: 'farmstead', service: 'farm-work', label: '耕作', command: 'show-farm-work', commandLabel: '查看农事' }), '空格/E/回车执行·Esc返回', '仓储已经顶满，先回去清仓再安排采收与外出。', '要务：仓储已满', 'facility.storage-chest').assetId).toBe('loc.farmstead');

    expect(locationSelectionToastPresentation('服务', location({ id: 'farmstead', displayName: '农庄' }), service({ locationId: 'farmstead', service: 'farm-work', label: '耕作', command: 'show-farm-work', commandLabel: '查看农事' }), '空格/E/回车执行·Esc返回', '有 1 块灵田正缺水，先回农庄把当日水路补稳。', '要务：补水 1 块灵田', 'loc.herb-plot').assetId).toBe('loc.herb-plot');
  });
  ...
  it('keeps farmstead root service toasts on the farmstead thread when only a sealing cabinet is ready', () => {
    expect(locationSelectionToastPresentation('服务', location({ id: 'farmstead', displayName: '农庄' }), service({ locationId: 'farmstead', service: 'farm-work', label: '耕作', command: 'show-farm-work', commandLabel: '查看农事' }), '空格/E/回车执行·Esc返回', '农庄里已有设施完工，先回去把这一轮产出收住。', '要务：待收设施 1 座', 'loc.farmstead').assetId).toBe('loc.farmstead');
  });
  ...
  it('keeps farmstead-root processing and array selection toasts on the farmstead thread until the player enters the child place', () => {
    expect(locationSelectionToastPresentation('服务', location({ id: 'farmstead', displayName: '农庄' }), service({ locationId: 'farmstead', service: 'processing', label: '农庄加工', command: 'show-processing', commandLabel: '查看加工' }), '空格/E/回车执行·Esc返回').assetId).toBe('loc.farmstead');

    expect(locationSelectionToastPresentation('服务', location({ id: 'farmstead', displayName: '农庄' }), service({ locationId: 'farmstead', service: 'arrays', label: '阵法布设', command: 'show-arrays', commandLabel: '查看阵法' }), '空格/E/回车执行·Esc返回').assetId).toBe('loc.farmstead');

    expect(locationSelectionToastPresentation('服务', location({ id: 'drying-yard', displayName: '晾晒架旁' }), service({ locationId: 'drying-yard', service: 'processing', label: '农庄加工', command: 'show-processing', commandLabel: '查看加工' }), '空格/E/回车执行·Esc返回').assetId).toBe('loc.drying-yard');

    expect(locationSelectionToastPresentation('服务', location({ id: 'array-shed', displayName: '阵器棚' }), service({ locationId: 'array-shed', service: 'arrays', label: '阵法布设', command: 'show-arrays', commandLabel: '查看阵法' }), '空格/E/回车执行·Esc返回').assetId).toBe('loc.array-shed');

    expect(locationSelectionToastPresentation('服务', location({ id: 'creek-field', displayName: '溪边药田' }), service({ locationId: 'creek-field', service: 'encounter', label: '偶遇', command: 'show-location-encounter', commandLabel: '查看偶遇' }), '空格/E/回车执行·Esc返回').assetId).toBe('loc.creek-field');
  });

  it('keeps commission-thread selection toasts anchored to the current place thread instead of forcing market art', () => {
    expect(locationSelectionToastPresentation('服务', location({ id: 'valley-market', displayName: '山谷集市' }), service({ locationId: 'valley-market', service: 'commission-board', label: '委托', command: 'show-commission', commandLabel: '查看委托' }), '空格/E/回车执行·Esc返回').assetId).toBe('loc.valley-market');

    expect(locationSelectionToastPresentation('服务', location({ id: 'ruin-gate', displayName: '遗迹门口' }), service({ locationId: 'ruin-gate', service: 'commission-board', label: '委托', command: 'show-commission', commandLabel: '查看委托' }), '空格/E/回车执行·Esc返回').assetId).toBe('loc.ruin-gate');
  });

  it('falls back to location asset when the selected location has no service', () => {
    expect(locationSelectionToastPresentation('地点', location({ id: 'herb-plot', displayName: '露根药圃', npcs: [] }), null, '空格/E/回车执行·Esc返回')).toEqual({
  ...
  it('keeps service-unavailable failures anchored to the current location', () => {
    expect(locationServiceUnavailableToastPresentation(location({ id: 'tea-shed', displayName: '旧茶棚' }))).toEqual({
  ...
  it('derives service-unavailable guidance from onboarding route locks', () => {
    ...
    state.player.flags.add('onboarding-first-shipping-settlement');
    expect(locationServiceUnavailableToastPresentation(location({ id: 'valley-outskirts', displayName: '山谷' }), state)).toEqual({
      message: '山谷当前先放一放｜先去集市补种，把第二轮炼丹材料接上',
      assetId: 'loc.valley-outskirts'
    });

    state.player.flags.add('onboarding-first-market-restock');
    expect(locationServiceUnavailableToastPresentation(location({ id: 'ruin-gate', displayName: '遗迹门口' }), state)).toEqual({
  ...
  it('keeps no-encounter failures anchored to the current location instead of a generic string', () => {
    expect(locationEncounterUnavailableToastPresentation(location({ id: 'creek-field', displayName: '溪边药田' }))).toEqual({
