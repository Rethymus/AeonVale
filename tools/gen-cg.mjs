#!/usr/bin/env node
/**
 * 结局 CG 生成（docs/13 §1.3：AI 生成图唯一可直接进游戏的类别）。
 *
 * 用法：
...
for (const r of results) {
  if (r.ok) {
    console.log(
      JSON.stringify(
        { id: r.id, path: r.file.replace(/^assets\//, ''), type: 'png', checksum: r.sha, license: 'AI-Generated', source: `gpt-image-2 via relay; prompt: ${r.prompt.slice(0, 60)}…` },
        null, 0,
      ),
    );
  }
