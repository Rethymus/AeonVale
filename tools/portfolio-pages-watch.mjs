...
function printReport(report) {
  ...
  if (report.ciRun && !report.ciRun.error) console.log(`Latest CI: #${report.ciRun.databaseId} ${report.ciRun.conclusion}/${report.ciRun.status} ${report.ciRun.headSha ?? ''}`);
  if (report.pagesRun && !report.pagesRun.error) console.log(`Latest Pages Action: #${report.pagesRun.databaseId} ${report.pagesRun.conclusion}/${report.pagesRun.status} ${report.pagesRun.headSha ?? ''}`);
...
async function main() {
  ...
  report.watch = { enabled: wait, settled: isSettled(report), elapsedMs: Date.now() - started, intervalMs, timeoutMs: maxWaitMs };
