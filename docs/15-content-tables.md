...
设计覆盖：寒/热/温/平四属性齐全；各 tier 至少 2 种；含**金属性引雷草**（高 `metalAttract`，作活引雷针）与**核心脆弱药草**（高药性、高丹毒风险）。`metalAttract` 字段对应 14 §5 `lightning.metalAttract`（系数 P012=0.8 × tier）。

| id | 中文名 | English | 药性向量 `[cold,hot,warm,neutral]` | tier | baseGrowth (G/日) | growthThreshold | qiNeed | qiDrain/日 | metalAttract | 引雷属性 | 季节亲和 | 种子来源 | 简介 / 设计意图 |
|----|--------|---------|-----------------------------------|------|-------------------|-----------------|--------|-----------|--------------|----------|----------|----------|------------------|
...
| `herb.frostmarrow` | 寒潭莲 | Frostmarrow | `[6,0,2,0]` | 2 | 5 | 80 | 20 | 1.2 | 0 | 无 | 冬 | 储物戒残卷 | 强寒性，承雷丹主料；生食丹毒高 |
...
| `herb.metalpine` | 雷击木 | Metalpine | `[1,1,0,2]` | 3 | 4 | 120 | 35 | 1.6 | **3.2** | **吸雷** | 秋 | 妖兽战利品/实验 | **金属性引雷草**：高 metalAttract，种田即布阵（14 §5 核心） |
| `herb.thunderreed` | 引雷芦 | Thunderreed | `[2,0,0,3]` | 3 | 4 | 130 | 32 | 1.5 | **2.4** | 吸雷 | 春 | 天象"雷汛"掉落 | 第二种引雷草，寒性可双用 |
...
| `herb.voidmantle` | 虚衾蕈 | Voidmantle | `[7,0,1,1]` | 4 | 3 | 210 | 58 | 2.4 | 0 | 无 | 冬 | 灵脉深掘 | 极寒，高阶承雷丹/淬体料 |
| `herb.solar-pith` | 太阳髓 | Solar Pith | `[0,7,1,1]` | 4 | 3 | 210 | 58 | 2.4 | 0 | 无 | 夏 | 灵脉深掘 | 极热，对应虚空衾 |
| `herb.ironwill-thorn` | 铁心刺 | Ironwill Thorn | `[0,0,0,5]` | 4 | 3 | 220 | 52 | 2.0 | **4.0** | **强吸雷** | 平 | 阵法残谱兑换 | 最强引雷草，导雷核心布阵件 |
...
> **设计校验**：`metalAttract` 列已按 14 P012 (`0.8 × tier`) 校准——Metalpine tier3 = 0.8×3=2.4，表中取 3.2 为其"特化导雷"加成（超出基线的设计特性，在机制文档登记为 `metalAttractBonus`）。Ironwill Thorn tier4 基线 3.2 + 特化 = 4.0。**[待与 05 对齐：特化加成是否独立字段]**

---

## 2. 丹方目录 Recipes

设计覆盖：承雷丹/生骨丹/净毒丹/淬体丹四大备劫刚需 + 若干**涌现配方**（标注 `emergent`）。`idealHeat` 区间对应 14 §9 `extraction` 拱形；`difficulty` 影响平衡容差球半径。

| id | 中文名 | 输入材料 + 数量 | 理想火候区间 `[min,max]` | 目标药性向量 | 产出丹药 | 难度 | 解锁方式 | 简介 / 设计意图 |
|----|--------|-----------------|--------------------------|--------------|----------|------|----------|------------------|
| `recipe.ward-pill` | 承雷丹方 | Metalpine×1 + Frostmarrow×1 + Balmleaf×1 | `[35, 55]` | `[4,0,1,2]` | `pill.ward-basic` | 2 | 储物戒残卷 | 承雷主药，新手第一颗意义丹 |
...
| `recipe.ward-greater` | 大承雷丹方 | Ironwill Thorn×1 + Voidmantle×1 + Boneash Lily×1 | `[45, 65]` | `[6,1,2,3]` | `pill.ward-greater` | 4 | 残卷 + stage3 | stage3+ 承雷主药 |
...
| `recipe.ward-heaven` | 偷天承雷方 | Ironwill Thorn×1 + Violet Ascendshroom×1 + Dao Root×1 | `[50, 70]` | `[5,5,5,8]` | `pill.ward-heaven` | 5 | stage5 + 飞升线 | 终极承雷，仅终局可成 |
...
| `recipe.emergent.storm-seed` | 雷种（涌现） | Metalpine×2 + Thunder Reed×1（中火双金属） | `[45, 55]` | `[3,0,0,5]` | `pill.storm-seed` | 3 | **emergent**（双引雷草合炼） | 种下后长出"雷草"，雷场陷阱件 |
...
| id | 中文名 | 效果 | 持续时间 | 副作用 | 丹毒负荷 `load` | 备劫用途 | tier |
|----|--------|------|----------|--------|-----------------|----------|------|
| `pill.ward-basic` | 承雷丹 | 单次承雷减伤 0.4（14 P020） | 下一次天劫 | 无 | 5 | stage1–2 承雷 | 2 |
...
| `pill.ward-greater` | 大承雷丹 | 单次承雷减伤 0.6 | 下一次天劫 | 无 | 7 | stage3–4 承雷 | 4 |
...
| `pill.ward-heaven` | 偷天承雷丹 | 单次承雷减伤 0.75 | 下一次天劫 | 无 | 12 | 终极承雷 | 5 |
...
| `pill.storm-seed` | 雷种 | 使用后原地长出雷草（雷场陷阱，吸雷+小伤） | 种植物 | 无 | 3 | 导雷布阵件 | 3 |
...
|----|--------|------|-----------------|-----------|----------|-----------|--------------|--------------|
| `event.qi-tide` | 灵气潮汐 | 喜 | stage≥1 | 10 | 5 日 | 灵气暴涨→灵草翻倍成熟→引来妖兽 | `celestialQiMod=1.5`；生长 ×1.5 | 抢收 + 护田巡守 |
...
| `event.heaven-eye` | 天道注视 | 危 | stage≥3, 临近突破 | 5 | 当次天劫 | 天劫强度 +30% | `bolt.baseDamage ×1.3` | 务必备大承雷丹 |
| `event.demon-seed-rain` | 魔种雨 | 机+危 | stage≥3 | 4 | 1 日 | 天降异种→可种但风险 | 解锁 Griefvein 种子；伴生妖兽 | 风险投资 |
| `event.vein-surfacing` | 残脉露头 | 机 | stage≥2 | 3 | 永久（挖掘后） | 地下灵脉露头可掘 | 新增 `veinMultiplier=3.0` 地块 | 优先抢占/巡守 |
| `event.body-ruin` | 体修遗迹 | 机+危 | stage≥2, 体魄进度≥60% | 2 | 1 次 | 无宗门传承的残破炼体遗迹现世→可得体修残页/淬体方 | 掉落体修残页；探索失败则 HP/丹毒惩罚 | 备生骨丹，权衡奇遇与伤势 |
| `event.lightning-storm` | 雷汛 | 危 | stage≥3, 夏 | 5 | 3 日 | 非天劫期散雷→伤人伤草 | 随机小雷（`baseDamage×0.3`） | 引雷草/雷种收集窗 |
...
| `item.insulate-mat` | 绝缘阵席 | Insulating Mat | 阵件 | 布设绝缘阵（`arrayModifier 0.3`，乘性正倍率，越小越隔绝，见 14 P014） | 4 | stage2 制造 | 稳住核心区 |
...
| `item.guard-beast-whistle` | 守田兽哨 | Guardian Beast Whistle | 工具 | 指派守田兽巡逻地块 | 1 | 守田兽归巢事件 | 现代"看门狗"思路的护田工具 |
...
| 引雷草（metalAttract>0） | Metalpine/Thunderreed/Fulgurseed/Ironwill Thorn + Violet/Boneash(半) = 6 种 ✔ |
| 丹方覆盖四大备劫刚需 | 承雷/生骨/净毒/淬体 全覆盖且分 tier 进阶 ✔ |
