...
async function loadRenderAssets(store: AssetStore): Promise<RuntimeRenderAssets> {
 ...
 const facilityIds = store.list('sprites')
 .map((entry) => entry.id)
 .filter((id) => id.startsWith('facility.'));
 const locationIds = store.list('sprites')
 .map((entry) => entry.id)
 .filter((id) => id.startsWith('loc.'));
 const tileIds = store.list('sprites')
 .map((entry) => entry.id)
 .filter((id) => id.startsWith('tile.'));
 const logoIds = store.list('sprites')
 .map((entry) => entry.id)
 .filter((id) => id.startsWith('logo.'));
 const allNpcAssetIds = runtimeNpcAssetIds(store);
 const hotbarIconIds = [...new Set(HOTBAR_SLOTS
 .map((slot) => hotbarSlotAssetId(slot))
 .filter((id): id is string => Boolean(id)))];

const ids = [
 'sprite.player',
 'cg.ending-ascension',
 'cg.ending-lifespan-death',
 'cg.ending-poison-death',
 ...GUARD_BEAST_ASSET_IDS,
 ...allNpcAssetIds,
 ...facilityIds,
 ...hotbarIconIds,
 ...locationIds,
...
return {
 ...
 facilityIds.map((id) => [id.slice('facility.'.length), textureById.get(id)]),
 ),
 endingCg: {
...
function spawnFarmActionBurst(kind: FarmActionFeedbackKind, affectedTiles: ReadonlyArray<{ x: number; y: number }>): void {
 const style = kind === 'till'
 ? { color: 0xd8b070, count: 8, speed: 2.1 }
 : kind === 'water'
 ? { color: 0x7ec8ff, count: 8, speed: 2.4 }
 : kind === 'harvest'
 ? { color: 0xffe066, count: 12, speed: 2.8 }
 : { color: 0x66ddff, count: 10, speed: 2.6 };
...
spawnFarmActionBurst(kind, outcome.affectedTiles);
 if (kind === 'till') audio.playSfx('till'); // G4: 翻地音效
...
function publishDebugSnapshot(): void {
 ...
 const shippingChoices = interactionPanel.kind === 'shipping'
 ? interactionPanel.mode === 'normal'
 ? normalShipChoices()
 : qualityShipChoices()
 : [];
 const selectedShippingChoice = interactionPanel.kind === 'shipping'
 ? interactionPanel.mode === 'normal'
 ? (shippingChoices[normalizeSelection(shipIdx, shippingChoices.length)] ?? null)
 : (shippingChoices[normalizeSelection(qualityShipIdx, shippingChoices.length)] ?? null)
 : null;
...
function activateLocationSelection(prefix: '地点' | '服务'): void {
 ...
 const selectedService = services.length > 0 ? (services[locationServiceIdx % services.length] ?? null) : null;
 const presentation = locationSelectionToastPresentation(
 prefix,
 location,
 selectedService,
	locationSelectionHint(),
 locationPreviewFocusReason(
 state,
 getOnboardingObjectiveId(state),
 location.id,
 selectedService?.command ?? null,
 getLocationEncounters(state, location.id).length,
 ),
 formatLocationActionSignalLine(state, location.id),
 location.id === 'farmstead' && selectedService?.command === 'show-farm-work'
 ? farmsteadRootContextAssetId(state)
 : undefined,
 );
 toast(presentation.message, presentation.assetId);
 }
...
function toggleInventoryVisibility(): void {
 ...
 {
 const presentation = overlayToastPresentation(
 layers.showInv ? 'open-inventory' : 'close-inventory',
	inventoryOverlayAssetId(),
 );
 toast(presentation.message, presentation.assetId);
 }
 }
...
function openFarmActionPanel(): void {
 ...
 const presentation = farmActionMenuToastPresentation(
 kind,
 selectionLabel(farmActionIdx, FARM_ACTION_ORDER.length),
 confirmHint('进入'),
 state,
 ctx,
 );
...
function openNpcActionPanel(): void {
 ...
 const presentation = npcActionMenuToastPresentation(
 mode,
 selectionLabel(npcActionIdx, NPC_ACTION_ORDER.length),
 confirmHint('进入'),
 );
 toast(presentation.message, presentation.assetId);
 }
...
function openProcessingPanel(mode: 'drying' | 'sealing' | 'furnace'): void {
 ...
 const presentation = processingToastPresentation(
 'drying',
 describeStorageChoice(choice),
 selectionLabel(processingIdx, choices.length),
 confirmHint('开始'),
 );
 toast(presentation.message, presentation.assetId);
 return;
...
function openFacilityCollectPanel(): void {
 ...
 }, selectionLabel(facilityCollectIdx, choices.length), confirmHint('收取'));
 toast(presentation.message, presentation.assetId);
 }
...
function openNpcPanel(mode: NpcPanelMode): void {
 ...
 const itemId = bestGiftItemForNpc(state, npc.id);
 const name = itemId ? (reg.items.get(itemId)?.displayName ?? itemId) : null;
 const presentation = npcGiftToastPresentation(
 npc,
 name,
 Boolean(schedule?.birthday),
 selectionLabel(npcIdx, npcs.length),
 confirmHint(itemId ? '赠礼' : '尝试赠礼'),
 itemId,
 reg,
 );
 toast(presentation.message, presentation.assetId);
 ...
 interactionPanel = { kind: 'npc', mode };
 const presentation = npcQuestToastPresentation(
 npc,
 npcQuest,
 selectionLabel(npcIdx, npcs.length),
 confirmHint(npcQuest ? '推进' : '尝试领取'),
 reg,
 );
 toast(presentation.message, presentation.assetId);
 }
...
function participateFestivalWithToast(): void {
 ...
 const presentation = festivalUnavailableToastPresentation(
 state.activeEvent?.defId?.endsWith('-festival') ? 'already-participated-or-full' : 'no-active-event',
 );
 toast(presentation.message, presentation.assetId);
 }
...
function selectedPanelItemPreview(): { title: string; details: string; texture?: Texture } | null {
 ...
 const idx = interactionPanel.mode === 'deposit'
 ? normalizeSelection(storageDepositIdx, choices.length)
 : normalizeSelection(storageWithdrawIdx, choices.length);
 const choice = choices[idx]!;
 const preview = storagePanelPreview(
 interactionPanel.mode,
 choice,
 storageUsed(state.storage),
 ...
 const preview = shippingPanelPreview('normal', {
 ...choice,
 unitPrice: shippingUnitPrice(ctx, choice.itemId, undefined, state),
 }, reg);
 return {
 ...
 const preview = shippingPanelPreview('quality', {
 ...choice,
 unitPrice: shippingUnitPrice(ctx, choice.itemId, choice.quality, state),
 }, reg);
 return {
...
function showCommission(): void {
 ...
 const activeOrder = getActiveSpecialOrders(state)[0] ?? null;
 if (activeOrder) {
 const presentation = commissionToastPresentation(
 activeSpecialOrderPanelPreview(activeOrder, reg, '镇守事务'),
 '镇守告示',
 confirmHint('提交或领奖'),
 );
 toast(presentation.message, presentation.assetId);
 ...
 if (incident && !hasResolvedStayingWorldIncidentForDay(state, state.day)) {
 const presentation = commissionToastPresentation(
 stayingWorldIncidentPanelPreview(incident, reg),
 '镇守告示',
 confirmHint('处置'),
 );
 toast(presentation.message, presentation.assetId);
 ...
 const presentation = commissionToastPresentation(
 dailyCommissionPanelPreview(commission, reg, '镇守差事', itemCount(state.player, commission.request.itemId)),
 '镇守告示',
 confirmHint('交付'),
 );
 toast(presentation.message, presentation.assetId);
 ...
 if (mainline) {
 const presentation = commissionToastPresentation(
 mainlineQuestPanelPreview(mainline, reg),
 '告示板',
 confirmHint('推进'),
 );
 toast(presentation.message, presentation.assetId);
 ...
 if (ruinChapter) {
 const presentation = commissionToastPresentation(
 ruinChapterPanelPreview(ruinChapter, state.exploration.deepestRuinLevel, reg),
 '告示板',
 confirmHint('推进'),
 );
 toast(presentation.message, presentation.assetId);
 ...
 if (activeOrder) {
 const presentation = commissionToastPresentation(
 activeSpecialOrderPanelPreview(activeOrder, reg, '特别订单', itemCount(state.player, activeOrder.request.itemId)),
 '告示板',
 confirmHint('提交或领奖'),
 );
 toast(presentation.message, presentation.assetId);
 ...
 if (specialOrder) {
 const presentation = commissionToastPresentation(
 dailySpecialOrderPanelPreview(specialOrder, reg, '待接特别订单', itemCount(state.player, specialOrder.request.itemId)),
 '告示板',
 confirmHint('接取'),
 );
 toast(presentation.message, presentation.assetId);
 ...
 const presentation = commissionToastPresentation(
 dailyCommissionPanelPreview(commission, reg, '公告委托', itemCount(state.player, commission.request.itemId)),
 '告示板',
 confirmHint('交付'),
 );
 toast(presentation.message, presentation.assetId);
...
function confirmInteractionPanel(): boolean {
 ...
 if (result.ok) {
 audio.playSfx('ui');
 const detail = upgrade.inventoryCapacityBonus
 ? `储物戒容量 ${state.player.inventoryCapacity}`
 : upgrade.id.startsWith('greenhouse-nursery-')
 ? '棚温更稳，暖棚养护收益提高'
 : upgrade.toolStaminaMult
 ? '农具更省力'
 : undefined;
 ...
 audio.playSfx('ui');
 const presentation = npcQuestResultToastPresentation(
 npc,
 npcQuest,
 payload.nextQuestTitle ? 'advance' : 'complete',
 reg,
 payload.nextQuestTitle,
 );
 toast(presentation.message, presentation.assetId);
 } else {
...
const commission = getDailyCommission(state);
 ...
 const choice = interactionPanel.mode === 'deposit'
 ? choices[storageDepositIdx % choices.length]!
 : choices[storageWithdrawIdx % choices.length]!;
 const eventStart = state.events.length;
 if (interactionPanel.mode === 'deposit') {
 ...
 const presentation = facilityCollectFailureToastPresentation(
 choice.kind === 'drying-rack' ? 'drying' : choice.kind === 'sealing-cabinet' ? 'sealing' : 'furnace',
 { reason: '尚未完成或背包已满' },
 );
 toast(presentation.message, presentation.assetId);
 }
...
function locationPreviewDetails(): { title: string; details: string; texture?: Texture; npcPrimary?: Texture; npcSecondary?: Texture } | null {
 ...
 const previewThreadLocation = location.id === previewThreadLocationId
 ? location
 : {
...
function showLocationEncounter(locationId: LocationId): void {
 ...
 ? locationEncounterUnavailableToastPresentation(location)
 : locationDirectoryEmptyToastPresentation(locationPreviewAssetId(locationId));
 toast(presentation.message, presentation.assetId);
 return;
 }
...
function openTeaShedPanel(): void {
 ...
 const presentation = teaShedToastPresentation(
 state,
 state.postAscension.mode === 'stayed-in-world' ? confirmHint('歇脚听闻') : undefined,
 );
 toast(presentation.message, presentation.assetId);
 }
...
function openGreenhousePanel(): void {
 ...
 const presentation = greenhouseToastPresentation(
 state,
 reg,
 state.postAscension.mode === 'stayed-in-world' ? confirmHint('养护暖棚') : undefined,
 );
 toast(presentation.message, presentation.assetId);
 }
...
function executeLocationCommand(command: LocationServiceCommand, locationId: LocationId): void {
 ...
 {
 const presentation = processingServiceToastPresentation(
 confirmHint('进入').replace('·Esc返回', ''),
 locationId,
 );
 toast(presentation.message, presentation.assetId);
 }
 ...
 case 'show-farm-work':
 {
 const presentation = farmWorkServiceToastPresentation(
 '空格/E主交互·M开农庄操作',
 locationId === 'farmstead' ? farmsteadRootContextAssetId(state) : undefined,
 );
 toast(presentation.message, presentation.assetId);
 }
 return;
...
function endDay(): void {
 ...
 const summary = daySummaryPresentation(
 state.day,
 state.events,
	ctx.content,
 readyForBreakthrough(state, DEFAULT_BALANCE),
 currentOnboardingHelpText(),
 );
 const objectiveAfter = getOnboardingObjectiveId(state);
...
function shouldAutoConfirmLocationPanel(): boolean {
 return interactionPanel.kind === 'commission'
 || interactionPanel.kind === 'tea-shed'
 || interactionPanel.kind === 'greenhouse';
 }
...
function performLegacyConfirmShortcut(): void {
 ...
 const presentation = legacyConfirmUnavailableToastPresentation(
 interactionPanelCloseAssetId(interactionPanel)
	?? locationSelectionContextAssetId()
 ?? farmsteadRootContextAssetId(state),
 );
 toast(presentation.message, presentation.assetId);
 }
...
window.addEventListener('keydown', (ev) => {
 ...
 const enterShortcut = ev.key === 'Enter'
 ...
 : null;
 const escapeShortcut = ev.key === 'Escape'
 ...
 : null;
 const qShortcut = (ev.key === 'q' || ev.key === 'Q')
 ? resolveQShortcut({
 ctrlKey: Boolean(ev.ctrlKey),
 shiftKey: Boolean(ev.shiftKey),
 quickLocationShortcut,
 ...
 const shouldDismissInteractionPanel = interactionPanelActive(interactionPanel)
 ...
 const shouldDismissLocationSelection = locationSelectionActive
 ...
 const presentation = guardBeastFeedResultToastPresentation(
 { ...choice, count: 1 },
 { beastId: payload.id, vigor: payload.vigor, bond: payload.bond },
 reg,
 );
 audio.playSfx('ui');
 toast(presentation.message, presentation.assetId);
...
app.ticker.add(() => {
 ...
 drawHotbarIcon(layers, renderAssets.hotbarIcons[hotbarSlotAssetId(hotbarSlot) ?? '']);
	refreshHelpHint();
 const focusedOverlayActive = locationSelectionActive
 || interactionPanelActive(interactionPanel)
 || layers.showInv
 || cultivationPanelVisible
 || paused
 ...
 hideLocationPreview(layers);
	...
	const panelPreview = !paused
	&& dialogueBeat === null
	&& !cultivationPanelVisible
	&& (interactionPanelActive(interactionPanel) || layers.showInv)
	? selectedPanelItemPreview()
	: null;
