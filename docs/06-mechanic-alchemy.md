...
> 兄弟文档：灵草药性来源见 `08-farming-system.md`；备劫丹药用途见 `05-mechanic-tribulation.md`；丹方内容表见 `15-content-tables.md`；数值收口见 `14-game-balance-and-math.md`。
...
3. **承雷出口**：炼丹的产出（承雷丹、生骨丹、净毒丹、淬体增效丹）直接服务于天劫。形成 `种田 → 炼丹 → 承雷淬体 → 突破` 的闭环。
...
```
Recipe {
  IdealHeatRange: (min, max)   // e.g. 承雷丹要求 (55, 70)
...
- 顺序 [金草, 水] + AvgHeat=45 → **承雷丹** (LightningWardPill)
...
| 丹药 | 功能 | 天劫用途 | 主要材料倾向 |
|------|------|---------|------------|
| `承雷丹 LightningWardPill` | 服后 60s 内雷伤减免 40–60% | 渡劫前服下 | 金属性草 + 水辅料 |
