...
describe('PBT-BEAST: 妖兽潮不变式 ', () => {
  ...
  it('PBT-BEAST-01: 触发后妖兽数 ∈ [countMin, countMaxBase+stage]', () => {
    fc.assert(
      fc.property(fc.integer({ min: 1, max: 100_000 }), fc.constantFrom(...STAGES), fc.integer({ min: 1, max: 8 }), (seed, stage, cropCount) => {
  ...
  it('PBT-BEAST-02a: 无灵气潮汐 → 永不触发（任意种子/作物数）', () => {
    fc.assert(
      fc.property(fc.integer({ min: 1, max: 100_000 }), fc.integer({ min: 0, max: 8 }), (seed, cropCount) => {
        const P = beastParams({ surgeChancePerDay: 1.0 });
  ...
  it('PBT-BEAST-05: 主动猎妖掉落数 ∈ [0, 击杀数] 且确定', () => {
    fc.assert(
      fc.property(fc.integer({ min: 1, max: 100_000 }), fc.integer({ min: 1, max: 5 }), (seed, beasts) => {
