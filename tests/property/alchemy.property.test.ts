...
describe('炼丹属性测试 ', () => {
  ...
  it('PBT-balanceScore: 有界 [0,1]', () => {
    fc.assert(
      fc.property(fc.record({ cold: fc.integer(), hot: fc.integer(), warm: fc.integer(), neutral: fc.integer() }), fc.record({ cold: fc.integer(), hot: fc.integer(), warm: fc.integer(), neutral: fc.integer() }), (a, b) => {
  ...
  it('PBT-炸炉确定性：同 (材料,火候,阶段) ⇒ 同 outcome（resolveBrew 纯函数）', () => {
    ...
    fc.assert(
      fc.property(fc.array(fc.record({ herbId: fc.constantFrom(...herbIds), qty: fc.integer({ min: 1, max: 3 }) }), { minLength: 1, maxLength: 4 }), fc.integer({ min: 0, max: 100_000 }), (materials, heat) => {
  ...
  it('PBT-药性聚合有界：不超各输入加权和（守恒）', () => {
    ...
    const state = createWorld({ seed: 1, width: 4, height: 4, content: reg, params: DEFAULT_BALANCE });
    fc.assert(
      fc.property(fc.array(fc.record({ herbId: fc.constantFrom(...herbIds), qty: fc.integer({ min: 1, max: 2 }) }), { minLength: 1, maxLength: 3 }), fc.integer({ min: 0, max: 100_000 }), (materials, heat) => {
