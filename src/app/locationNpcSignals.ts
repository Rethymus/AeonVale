...
export function collectLocationNpcSignals(
 state: GameState,
 encounters: readonly LocationEncounter[],
): LocationNpcSignals {
 return {
 birthdayNames: encounters.filter((entry) => entry.birthday).map((entry) => entry.npcName),
 questNames: encounters
 .filter((entry) => Boolean(getCurrentNpcQuest(state, entry.npcId)))
 .map((entry) => entry.npcName),
 questReadyNames: encounters
 .filter((entry) => Boolean(getCurrentNpcQuest(state, entry.npcId)?.completed))
 .map((entry) => entry.npcName),
 };
}
...
const strength = signalStrength(signals);
 ...
 && best
 && location.displayName.localeCompare(best.location.displayName, 'zh-CN') >= 0
 ) {
 continue;
 }
