...
function archiveRewardText(reward: ArchiveRewardLike, content: ContentRegistry): string {
 return [
 reward.itemId && reward.count ? `${itemName(reward.itemId, content)}×${reward.count}` : '',
 reward.bodyFoundation ? `体魄+${Math.floor(reward.bodyFoundation / 1000)}` : '',
 reward.willpower ? `意志+${Math.floor(reward.willpower / 1000)}` : '',
 ].filter(Boolean).join('、');
}
...
function commissionAssetId(
 itemId: string,
 npcId: string,
 content: ContentRegistry,
 mode: CommissionPreviewMode,
): string | undefined {
...
export function mainlineQuestPanelPreview(
 quest: MainlineQuestStatus,
 content: ContentRegistry,
): CommissionPanelPreview {
 return {
 title: quest.title,
...
export function ruinChapterPanelPreview(
 chapter: RuinChapterLike,
 deepestRuinLevel: number,
 content: ContentRegistry,
): CommissionPanelPreview {
 const progressLine = chapter.completed
 ? `已达 ${chapter.floorEnd} 层，可领取章节奖励`
 : `遗迹 ${deepestRuinLevel}/${chapter.floorEnd}｜目标 ${chapter.floorStart}-${chapter.floorEnd} 层`;
...
export function activeSpecialOrderPanelPreview(
...
export function dailySpecialOrderPanelPreview(
 order: SpecialOrderStatus,
 content: ContentRegistry,
 heading = '待接特别订单',
 ownedCount?: number,
...
export function dailyCommissionPanelPreview(
...
export function stayingWorldIncidentPanelPreview(
 incident: StayingWorldIncidentDef,
 content: ContentRegistry,
): CommissionPanelPreview {
 return {
...
export function commissionToastPresentation(
 preview: CommissionPanelPreview,
 prefix: string,
 confirmLabel: string,
...
export function commissionBoardEmptyToastPresentation(stayingWorldOnly = false): CommissionToastPresentation {
 ...
 ? '镇守告示今日暂无差事｜先巡农庄与会场动线，稍后再看'
 : '公告板眼下暂无委托｜先回农庄、集市或人物动线推进',
...
export function archiveDonationToastPresentation(
 title: string,
 reward: ArchiveRewardLike,
 content: ContentRegistry,
): CommissionToastPresentation {
...
export function archiveDonationFailureToastPresentation(
 request: ArchiveRequestLike,
 content: ContentRegistry,
): CommissionToastPresentation {
 return {
...
export function archiveMilestoneToastPresentation(
 title: string,
 reward: ArchiveRewardLike,
 content: ContentRegistry,
): CommissionToastPresentation {
...
export function commissionCompleteToastPresentation(
 commission: CommissionDef,
 content: ContentRegistry,
...
export function commissionIncompleteToastPresentation(
 commission: CommissionDef,
 content: ContentRegistry,
 stayingWorldOnly = false,
): CommissionToastPresentation {
 const preview = dailyCommissionPanelPreview(
 commission,
 content,
 stayingWorldOnly ? '镇守差事' : '公告委托',
 );
 return {
...
export function specialOrderAcceptToastPresentation(
 order: SpecialOrderStatus,
 content: ContentRegistry,
): CommissionToastPresentation {
...
export function specialOrderAcceptFailureToastPresentation(
 order: SpecialOrderStatus,
 content: ContentRegistry,
): CommissionToastPresentation {
 const preview = dailySpecialOrderPanelPreview(order, content, '待接特别订单');
...
export function specialOrderClaimToastPresentation(
 order: SpecialOrderStatus,
 content: ContentRegistry,
 stayingWorldOnly = false,
): CommissionToastPresentation {
...
export function specialOrderClaimFailureToastPresentation(
 order: SpecialOrderStatus,
 content: ContentRegistry,
 stayingWorldOnly = false,
): CommissionToastPresentation {
...
export function specialOrderPendingToastPresentation(
 order: SpecialOrderStatus,
 content: ContentRegistry,
 stayingWorldOnly = false,
): CommissionToastPresentation {
...
export function specialOrderProgressToastPresentation(
 order: SpecialOrderStatus,
 progress: number,
 required: number,
 content: ContentRegistry,
...
export function specialOrderSubmitFailureToastPresentation(
 order: SpecialOrderStatus,
 content: ContentRegistry,
 stayingWorldOnly = false,
): CommissionToastPresentation {
...
export function stayingWorldIncidentResolveToastPresentation(
 incident: StayingWorldIncidentDef,
 content: ContentRegistry,
 resolved?: { beastId?: number },
): CommissionToastPresentation {
...
export function stayingWorldIncidentResolveFailureToastPresentation(
 incident: StayingWorldIncidentDef,
 content: ContentRegistry,
): CommissionToastPresentation {
 const preview = stayingWorldIncidentPanelPreview(incident, content);
...
export function mainlineQuestClaimToastPresentation(
 quest: MainlineQuestStatus,
 content: ContentRegistry,
...
export function mainlineQuestClaimFailureToastPresentation(
 quest: MainlineQuestStatus,
 content: ContentRegistry,
): CommissionToastPresentation {
 const preview = mainlineQuestPanelPreview(quest, content);
...
export function ruinChapterClaimToastPresentation(
 chapter: RuinChapterLike,
 deepestRuinLevel: number,
 content: ContentRegistry,
 nextChapter?: { title?: string | null; floorStart?: number | null; floorEnd?: number | null },
): CommissionToastPresentation {
 const preview = ruinChapterPanelPreview(chapter, deepestRuinLevel, content);
 const floorRange = nextChapter?.title
 ? `（${nextChapter.floorStart ?? '?'}-${nextChapter.floorEnd ?? '?'}层）`
 : '';
 return {
...
export function ruinChapterClaimFailureToastPresentation(
 chapter: RuinChapterLike,
 deepestRuinLevel: number,
 content: ContentRegistry,
): CommissionToastPresentation {
