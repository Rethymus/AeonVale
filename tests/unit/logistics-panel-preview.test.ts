...
const preview = storagePanelPreview(
 'deposit',
 { itemId: 'herb.mossling', count: 3, quality: 'spirit' },
 7,
 48,
 reg,
 );
...
const preview = shippingPanelPreview(
 'quality',
 { itemId: 'herb.dewroot', quality: 'treasure', count: 2, unitPrice: 6 },
 reg,
 );
...
expect(storagePanelPreview(
 'withdraw',
 { itemId: 'item.array-core', count: 1 },
 12,
 48,
 reg,
 ).details).toContain('取回要加工、炼丹、布阵或出货的物资');

expect(shippingPanelPreview(
 'normal',
 { itemId: 'item.dried-herb', count: 2, unitPrice: 4 },
 reg,
 ).details).toContain('回笼灵石，补种、炉料与备劫消耗不断档');
 });
...
expect(storagePanelPreview(
 ...
 48,
 reg,
 ).panelAssetId).toBe('loc.farmstead');

expect(shippingPanelPreview(
 'normal',
 { itemId: 'item.dried-herb', count: 1, unitPrice: 6 },
 reg,
 ).panelAssetId).toBe('loc.farmstead');
 });
...
expect(storageToastPresentation(
 'withdraw',
 { itemId: 'herb.mossling', count: 3, quality: 'spirit' },
 '（2/4）',
 '空格/E/回车取出·Esc返回',
...
expect(shippingToastPresentation(
 'quality',
 { itemId: 'herb.dewroot', quality: 'treasure', count: 2, unitPrice: 6 },
 '（1/3）',
 '空格/E/回车出货·Esc返回',
