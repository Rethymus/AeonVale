...
it('prioritizes tool breakage over worn warnings', () => {
 expect(toolFeedbackToast([
 event('tool-worn', { itemId: 'item.sickle', durability: 0 }),
 event('tool-broke', { itemId: 'item.sickle' }),
 ])).toBe('镰刀已损坏，先修补再续农务');
 expect(toolFeedbackToastPresentation([
 event('tool-worn', { itemId: 'item.sickle', durability: 0 }),
 event('tool-broke', { itemId: 'item.sickle' }),
 ])).toEqual({
...
it('falls back to the player thread when the tool is unknown', () => {
 expect(toolFeedbackToastPresentation([
 event('tool-broke', { itemId: 'item.unknown-tool' }),
 ])).toEqual({
 message: 'item.unknown-tool已损坏，先修补再续农务',
 assetId: 'sprite.player',
 });
 expect(toolFeedbackToastPresentation([
 event('tool-worn', { itemId: 'item.unknown-tool', durability: 2 }),
 ], 'loc.herb-plot')).toEqual({
 message: 'item.unknown-tool耐久仅剩 2，尽快修补以免断了药田节奏',
 assetId: 'loc.herb-plot',
 });
 expect(toolFeedbackToastPresentation([
 event('tool-broke', { itemId: 'item.unknown-tool' }),
 ], 'facility.shipping-bin')).toEqual({
 message: 'item.unknown-tool已损坏，先修补再续农务',
