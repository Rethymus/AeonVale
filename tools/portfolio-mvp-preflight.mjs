...
const steps = [
 ...
 {
 label: 'Capture portfolio screenshots and viewport checks',
 command: 'pnpm',
...
const portfolioScreenshots = [
 { path: 'test-results/portfolio/01-farm-loop.png', width: 960, height: 540 },
 { path: 'test-results/portfolio/02-location-routing.png', width: 960, height: 540 },
 { path: 'test-results/portfolio/03-farm-actions.png', width: 960, height: 540 },
 { path: 'test-results/portfolio/04-mobile-farm-loop.png', width: 960, height: 540 },
];
...
function cleanPortfolioScreenshots() {
 ...
 console.log('\n[portfolio:mvp-preflight] Cleared previous portfolio screenshots.');
}

function requireEvidenceText(content, text) {
 if (!content.includes(text)) {
 console.error(`[portfolio:mvp-preflight] Portfolio MVP evidence is missing required text: ${text}`);
 process.exit(1);
 }
}
...
function verifyScreenshotEvidence(evidence) {
 ...
 console.error('[portfolio:mvp-preflight] Portfolio MVP evidence must include screenshotEvidence paint stats.');
 process.exit(1);
 }
 for (const screenshot of portfolioScreenshots) {
 const entry = evidence.screenshotEvidence.find((item) => item?.path === screenshot.path);
 if (!entry) {
 console.error(`[portfolio:mvp-preflight] Portfolio MVP evidence is missing screenshot stats for ${screenshot.path}.`);
 process.exit(1);
 }
 if (entry.width !== screenshot.width || entry.height !== screenshot.height) {
 console.error(`[portfolio:mvp-preflight] Portfolio MVP evidence screenshot stats have unexpected dimensions for ${screenshot.path}: expected ${screenshot.width}x${screenshot.height}, got ${entry.width}x${entry.height}.`);
 process.exit(1);
 }
 ...
 console.error(`[portfolio:mvp-preflight] Portfolio MVP evidence screenshot stats must record the paint thresholds for ${screenshot.path}.`);
 process.exit(1);
 }
 if (entry.paintStats?.sampled <= portfolioPaintThresholds.minSampled) {
 console.error(`[portfolio:mvp-preflight] Portfolio MVP evidence screenshot stats have too few sampled pixels for ${screenshot.path}.`);
 process.exit(1);
 }
 if (entry.paintStats?.paintedRatio <= portfolioPaintThresholds.minPaintedRatio) {
 console.error(`[portfolio:mvp-preflight] Portfolio MVP evidence screenshot stats are too blank for ${screenshot.path}.`);
 process.exit(1);
 }
 if (entry.paintStats?.colors <= portfolioPaintThresholds.minColors) {
 console.error(`[portfolio:mvp-preflight] Portfolio MVP evidence screenshot stats have too few colors for ${screenshot.path}.`);
 process.exit(1);
 }
 }
}
...
function verifyPortfolioEvidence() {
 ...
 console.error(`[portfolio:mvp-preflight] Missing generated portfolio MVP evidence: ${portfolioEvidencePath}`);
 process.exit(1);
 }
 ...
 console.error(`[portfolio:mvp-preflight] Portfolio MVP evidence is not valid JSON: ${portfolioEvidencePath}`);
 process.exit(1);
 }
 if (evidence.generatedBy !== 'portfolio:capture') {
 console.error('[portfolio:mvp-preflight] Portfolio MVP evidence must be generated by portfolio:capture.');
 process.exit(1);
 }
 if (evidence.priority !== 'P0-A') {
 console.error('[portfolio:mvp-preflight] Portfolio MVP evidence must remain scoped to P0-A local review.');
 process.exit(1);
 }
 if (evidence.runtimeSignals?.onboardingObjectiveId !== 'first-loop-complete') {
 console.error('[portfolio:mvp-preflight] Portfolio MVP evidence must prove the first-loop onboarding objective is complete.');
 process.exit(1);
 }
 if (evidence.runtimeSignals?.firstLoopProgress !== '10/10') {
 console.error('[portfolio:mvp-preflight] Portfolio MVP evidence must prove first-loop progress is 10/10.');
 process.exit(1);
 }
 if (evidence.runtimeSignals?.selectedLocationId !== 'farmstead') {
 console.error('[portfolio:mvp-preflight] Portfolio MVP evidence must prove the showcase starts at the farmstead.');
 process.exit(1);
 }
 if (evidence.runtimeSignals?.selectedLocationServiceCommand !== 'show-farm-work') {
 console.error('[portfolio:mvp-preflight] Portfolio MVP evidence must prove the farm-work service is selected.');
 process.exit(1);
 }
 if (evidence.runtimeSignals?.shippingBinItemCount !== 2) {
 console.error('[portfolio:mvp-preflight] Portfolio MVP evidence must prove the shipping bin has showcase output.');
 process.exit(1);
 }
 if (evidence.runtimeSignals?.todayBriefingTitle !== '今日简报' || evidence.runtimeSignals?.todayBriefingHasAsset !== true) {
 console.error('[portfolio:mvp-preflight] Portfolio MVP evidence must prove today briefing is visible with an asset.');
 process.exit(1);
 }
 ...
 console.error('[portfolio:mvp-preflight] Portfolio MVP evidence must prove today briefing body carries the P0 farm, alchemy, tribulation, and 10/10 progress cues.');
 process.exit(1);
 }
...
console.log(`\n[portfolio:mvp-preflight] Verified ${portfolioScreenshots.length} generated portfolio screenshots.`);

verifyPortfolioEvidence();
console.log(`\n[portfolio:mvp-preflight] Verified generated portfolio MVP evidence: ${portfolioEvidencePath}.`);
...
console.log('\n[portfolio:mvp-preflight] Portfolio MVP preflight passed. No deployment, commit, or push was performed.');
