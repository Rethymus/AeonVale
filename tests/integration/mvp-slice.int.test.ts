...
import { advanceDay, applyAction, applyMvpStarterKit, createWorld, createSimContext, getOnboardingObjectiveId, simulateDay, DEFAULT_BALANCE, tileAt } from '@sim';
...
function setupPortfolioNewGame(seed = 20260710) {
  const reg = buildRegistry();
  const state = createWorld({ seed, width: 8, height: 8, content: reg, params: DEFAULT_BALANCE });
  const ctx = createSimContext(seed, reg, DEFAULT_BALANCE);
...
describe('MVP 垂直切片', () => {
  it('试玩首轮农务闭环：新档真实完成播种、照料、收获、出货、结算、补种', () => {
    ...
    expect(tileAt(state, at.x, at.y)?.cropId).not.toBeNull;
    expect(getOnboardingObjectiveId(state)).toBe('first-water');

    applyAction(state, { kind: 'water', at }, ctx);
    expect(tileAt(state, at.x, at.y)?.wateredToday).toBe(true);
    expect(getOnboardingObjectiveId(state)).toBe('first-harvest');

    const herbsBeforeHarvest = itemCount(state.player, 'herb.mossling');
    for (let day = 0; day < 20; day++) {
      const crop = state.crops.get(tileAt(state, at.x, at.y)!.id);
      if (crop?.stage === 'mature') break;
      advanceDay(state, ctx);
      const nextCrop = state.crops.get(tileAt(state, at.x, at.y)!.id);
      if (nextCrop?.stage !== 'mature') applyAction(state, { kind: 'water', at }, ctx);
    }

    expect(state.crops.get(tileAt(state, at.x, at.y)!.id)?.stage).toBe('mature');
    applyAction(state, { kind: 'harvest', at }, ctx);
    expect(itemCount(state.player, 'herb.mossling')).toBeGreaterThan(herbsBeforeHarvest);
    expect(getOnboardingObjectiveId(state)).toBe('first-ship');

    const spiritStonesBeforeSettlement = itemCount(state.player, 'item.spirit-stone');
    ...
    expect(itemCount(state.player, 'item.spirit-stone')).toBeGreaterThan(spiritStonesBeforeSettlement);
    ...
    applyAction(state, { kind: 'till', at: secondAt }, ctx);
    applyAction(state, { kind: 'sow', at: secondAt, seedId: 'seed.mossling' }, ctx);
    expect(getOnboardingObjectiveId(state)).toBe('first-second-water');

    applyAction(state, { kind: 'water', at: secondAt }, ctx);
  ...
  it('完整核心循环：翻地→种草→收获→炼丹→布阵→引劫→突破', () => {
    ...
    const tribResult = runTribulation(
      state,
      {
        stage: 1,
        boltCount: 3,
