...
export function createCultivationOpeningSurface(options: CultivationOpeningSurfaceOptions): CultivationStaticPhaseSurface {
  ...
  style.textContent = [
    '.cr-opening{height:100%;min-height:0;display:grid;grid-template-columns:minmax(0,1.08fr) minmax(320px,.92fr);overflow:hidden;color:var(--color-paperBright);background:radial-gradient(circle at 76% 18%,rgb(var(--rgb-giltUi) / .12),transparent 34%),rgb(var(--rgb-shellInk) / .92);}',
    '.cr-opening,.cr-opening *{box-sizing:border-box}',
    '.cr-opening__scene{position:relative;min-height:0;overflow:hidden;border-inline-end:1px solid rgb(var(--rgb-paperBorder) / .55);background:rgb(var(--rgb-shellPine) / .76);}',
    ...
    '.cr-opening__story{min-width:0;min-height:0;display:flex;flex-direction:column;padding:clamp(18px,3vw,38px);overflow:hidden;}',
    '.cr-opening__progress{display:grid;grid-auto-flow:column;grid-auto-columns:minmax(0,1fr);gap:8px;margin:0;padding:0;list-style:none;}',
    ...
    '.cr-opening__kicker{margin:clamp(16px,3vh,30px) 0 6px;color:var(--color-giltPale);font-size:11px;letter-spacing:.16em;}',
    '.cr-opening__title{margin:0;font-family:"Noto Serif CJK SC","Songti SC",serif;font-size:clamp(28px,3.5vw,48px);font-weight:600;line-height:1.13;letter-spacing:.05em;}',
    '.cr-opening__body{display:grid;gap:8px;margin:16px 0 0;color:var(--color-paperUi);font-size:clamp(13px,1.25vw,15px);line-height:1.68;}',
    '.cr-opening__body p{margin:0}',
    '.cr-opening__consequence{margin:14px 0 0;padding:9px 11px;border-inline-start:3px solid var(--color-giltUi);background:rgb(var(--rgb-shellPine) / .58);color:var(--color-paperBright);font-size:13px;line-height:1.5;}',
    '.cr-opening__actions{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-top:auto;padding-top:12px;}',
    ...
    '@container(max-width:760px){.cr-opening{grid-template-columns:1fr;grid-template-rows:minmax(170px,34%) minmax(0,1fr)}.cr-opening__scene{min-height:0;border-inline-end:0;border-block-end:1px solid rgb(var(--rgb-paperBorder) / .55)}.cr-opening__caption{inset-inline:12px;inset-block-end:8px;font-size:10px}.cr-opening__story{padding:12px 14px}.cr-opening__kicker{margin-top:10px}.cr-opening__title{font-size:clamp(22px,7vw,30px)}.cr-opening__body{margin-top:9px;font-size:12px;line-height:1.48}.cr-opening__consequence{margin-top:8px;padding:6px 8px;font-size:11px}.cr-opening__actions{padding-top:7px}.cr-opening__buttons{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));width:100%}.cr-opening__button{padding:7px 10px}}',
    '@media(max-height:700px){.cr-opening__body{font-size:12px;line-height:1.45}.cr-opening__consequence{font-size:11px;line-height:1.4}.cr-opening__caption{font-size:10px}}',
  ...
  function render(focus = true): void {
    ...
    next.textContent = beatIndex === beats.length - 1 ? '踏入第一道修途' : '继续';
