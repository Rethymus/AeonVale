...
it('三种自用消耗品收益谱各异：膏 hp > 茶 hp > 酒 hp；茶意志最高、膏无意志', () => {
 const wine = setup();
 mutateItem(wine.state.player, 'item.herbal-wine', 1);
 wine.state.player.hp = 50 * MILLI; wine.state.player.maxHp = 200 * MILLI; wine.state.player.pillPoison = 5 * MILLI;
 const w = consumeHerbalWine(wine.state, wine.ctx);

const tea = setup();
 mutateItem(tea.state.player, 'item.sealed-herb', 1);
 tea.state.player.hp = 50 * MILLI; tea.state.player.maxHp = 200 * MILLI; tea.state.player.pillPoison = 5 * MILLI;
 const t = offerRefinedTea(tea.state, tea.ctx);

const poultice = setup();
 mutateItem(poultice.state.player, 'item.spirit-poultice', 1);
 poultice.state.player.hp = 50 * MILLI; poultice.state.player.maxHp = 200 * MILLI; poultice.state.player.pillPoison = 5 * MILLI;
