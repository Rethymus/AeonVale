...
async function installSave(page: Page, mode: SaveFixtureMode) {
 ...
 return page.addInitScript(({ key, value }: { key: string; value: string }) => {
...
await clearIntroDialogue(page);
 ...
 if (!raw) return false;
 const parsed = JSON.parse(raw) as SaveSnapshot;
 return parsed.state?.postAscension?.mode === 'ascended-away'
 && parsed.state?.postAscension?.victoryRecorded === true
 && parsed.state?.ending === 'ascension'
 && parsed.state?.gameOver === true;
 }, SAVE_KEY);
