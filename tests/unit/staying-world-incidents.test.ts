...
import { STAYING_WORLD_INCIDENT_CATALOG } from '@sim/progression/stayingWorldIncidents';
...
describe('镇守事件目录 完整性', () => {
  it('全部事件 id 唯一（无重复，保护轮换与 resolve 的 find 语义）', () => {
    const ids = STAYING_WORLD_INCIDENT_CATALOG.map(incident => incident.id);
