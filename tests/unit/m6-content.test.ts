/**
 * M6 内容广度扩充—— 注册完整性 + 配方合法性。
 * brew 路径由 alchemy.test / content-lint 覆盖；本文件只断言新内容正确接入注册表。
 */
import { describe, it, expect } from 'vitest';
import { buildRegistry } from '@content/registry';

const NEW_HERBS = ['herb.stonegrain', 'herb.mistfern', 'herb.sunmoss', 'herb.plumeweed', 'herb.tidegrass', 'herb.embermoss', 'herb.jadewing', 'herb.fulgurseed'];
