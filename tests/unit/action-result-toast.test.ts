...
import {
 bodyTrainingToast,
 bodyTrainingToastPresentation,
 brewMaterialFailureToast,
 brewMaterialFailureToastPresentation,
 facilityCollectFailureToast,
 facilityCollectFailureToastPresentation,
 facilityCollectResultToast,
 facilityCollectResultToastPresentation,
 facilityFailureToast,
 facilityFailureToastPresentation,
 facilityJobStartToast,
 facilityJobStartToastPresentation,
 facilityStatusToast,
 facilityStatusToastPresentation,
 firstHarvestMilestoneToast,
 firstHarvestMilestoneToastPresentation,
 firstShipmentMilestoneToast,
 firstShipmentMilestoneToastPresentation,
 guardBeastFeedFailureToastPresentation,
 guardBeastFeedResultToast,
 guardBeastFeedResultToastPresentation,
 pillUseToast,
 pillUseToastPresentation,
 shippingFailureToast,
 shippingFailureToastPresentation,
 shippingResultToast,
 shippingResultToastPresentation,
 storageFailureToast,
 storageFailureToastPresentation,
 storageResultToast,
 storageResultToastPresentation,
} from '@app/actionResultToast';
...
expect(storageResultToast('deposit', { itemId: 'seed.mossling', count: 1 }, state, reg)).toBe(
 '存入仓库：凡间青苔种子×1｜仓占 2/48｜背包 0/16',
 );
 });
...
expect(shippingResultToast('normal', { itemId: 'item.dried-herb', count: 1 }, state, ctx, reg)).toBe(
 '投入出货箱：晾晒灵草×1｜本次 灵石×2｜可出 1 项｜已入箱 1 项',
 );

mutateQualityItem(state.player, 'herb.mossling', 'spirit', 1);
 state.qualityShippingBin.spirit = { 'herb.dewroot': 1 };

expect(shippingResultToast('quality', { itemId: 'herb.mossling', quality: 'spirit', count: 1 }, state, ctx, reg)).toBe(
 '投入出货箱：凡间青苔·灵品×1｜本次 灵石×4｜品质库存 1 项｜已入箱 1 项',
 );
 });
...
expect(storageFailureToast('deposit', { itemId: 'seed.mossling', count: 1 }, reg)).toBe(
 '存入失败：凡间青苔种子×1',
 );
...
expect(shippingFailureToast({ itemId: 'herb.mossling', quality: 'spirit', count: 1 }, reg)).toBe(
 '出货失败：凡间青苔·灵品×1',
 );
...
expect(facilityJobStartToast('drying', { itemId: 'herb.dewroot', count: 1, quality: 'treasure' }, {
 outputItemId: 'item.dried-herb',
 outputCount: 3,
 daysRemaining: 1,
 }, reg)).toBe('开始晾晒：露根草·珍品×1｜1日后得晾晒灵草×3｜可出货回灵石，也能接封藏、炼丹与阵法前置');

expect(facilityJobStartToast('sealing', null, {
 outputItemId: 'item.sealed-herb',
 outputCount: 1,
 daysRemaining: 2,
 }, reg)).toBe('开始封藏：2日后得封藏灵草×1｜可炼丹、交付或留作备劫药材');

expect(facilityJobStartToast('furnace', { itemId: 'item.spare-part', count: 1 }, {
 outputItemId: 'item.unknown-output',
 outputCount: 1,
 daysRemaining: 1,
 }, reg)).toBe('开始熔炼：item.spare-part×1｜1日后得item.unknown-output×1｜接上农庄加工循环');

expect(facilityCollectResultToast({ outputItemId: 'item.array-core', outputCount: 1 }, state, reg)).toBe(
 '收取设施：阵核×1｜可布阵，把农庄产出转成备劫防线｜背包 1/16',
 );
 });

it('keeps facility collect failures facility-led while no item has actually been received', () => {
 expect(facilityCollectFailureToast('drying', { reason: '尚未完成或背包已满' })).toBe(
 '晾晒架收取未成：尚未完成或背包已满',
 );
...
expect(firstHarvestMilestoneToast([
 { type: 'harvest', tick: 0, day: 1, payload: { defId: 'herb.mossling' } },
 ], reg, '下一步：把第一株灵草投进出货箱。')).toBe(
 '首轮收获：凡间青苔 已入手｜可炼丹、可出货，也是布阵备劫的第一份材料｜下一步：把第一株灵草投进出货箱。',
 );

expect(firstShipmentMilestoneToast(
 '投入出货箱：凡间青苔×1｜本次 灵石×1｜可出 1 项｜已入箱 1 项',
 '下一步：按 Enter 过夜，等次日出货结算。',
 )).toBe(
 '首轮投箱：投入出货箱：凡间青苔×1｜本次 灵石×1｜可出 1 项｜已入箱 1 项｜下一步：按 Enter 过夜，等次日出货结算。',
 );
...
expect(firstShipmentMilestoneToast(
 ...
 '首轮投箱：投入出货箱：凡间青苔×1｜本次 灵石×1｜可出 1 项｜已入箱 1 项｜下一步：按 Enter 过夜，等次日结算换回灵石。',
 );
 });
...
expect(guardBeastFeedResultToast(
 { itemId: 'herb.mossling', count: 1 },
 { beastId: 1, vigor: 7, bond: 12 },
 reg,
 )).toBe('投喂巡守兽：凡间青苔×1，精力7，羁绊12');

expect(guardBeastFeedResultToastPresentation(
 { itemId: 'herb.mossling', count: 1 },
 { beastId: 1, vigor: 7, bond: 12 },
 reg,
 ).assetId).toBe('sprite.guard-beast-boar');

expect(guardBeastFeedResultToastPresentation(
 { itemId: 'herb.mossling', count: 1 },
 { vigor: 7, bond: 12 },
 reg,
 ).assetId).toBe('sprite.guard-beast');
...
expect(pillUseToast('pill.ward-basic', { applied: true, effects: ['避雷护体25%'] }, reg)).toBe(
 '服 避雷丹：避雷护体25%｜备劫防线已补强',
 );

expect(pillUseToast('pill.bone-basic', { applied: true, effects: ['回血30'] }, reg)).toBe(
 '服 生骨丹：回血30｜续航和抗伤余量提高',
 );

expect(pillUseToast('pill.detox', { applied: true, effects: ['清毒25'] }, reg)).toBe(
 '服 净毒丹：清毒25｜丹毒压力下降，可继续炼丹或外出',
 );

expect(pillUseToast('pill.temper', { applied: true, effects: ['淬体×1.3'] }, reg)).toBe(
 '服 淬体丹：淬体×1.3｜下次天劫淬体收益提高',
 );
...
expect(storageResultToastPresentation('deposit', { itemId: 'seed.mossling', count: 1 }, state, reg).assetId).toBe('loc.farmstead');
 ...
 expect(firstHarvestMilestoneToastPresentation([
 { type: 'harvest', tick: 0, day: 1, payload: { defId: 'herb.mossling' } },
 ], reg, '下一步：把第一株灵草投进出货箱。')?.assetId).toBe('icon.herb.mossling');
 ...
 expect(guardBeastFeedResultToastPresentation(
 { itemId: 'herb.mossling', count: 1 },
 { beastId: 2, vigor: 9, bond: 5 },
 reg,
 ).assetId).toBe('sprite.guard-beast-wolf');
...
expect(storageResultToastPresentation('withdraw', { itemId: 'item.spirit-stone', count: 1 }, state, reg)).toMatchObject({
 ...
 expect(facilityJobStartToastPresentation('drying', { itemId: 'herb.dewroot', count: 1 }, {
 outputItemId: 'item.dried-herb',
 outputCount: 1,
 daysRemaining: 1,
 }, reg)).toMatchObject({
