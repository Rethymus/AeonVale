...
test('fresh player can complete the first farm economy loop from the first screen', async ({ page }) => {
 ...
 page,
 'Space',
 'second-round seed sown through the visible hotbar action',
 (debug) => debug.frontTileCropId != null
 && debug.onboardingObjectiveId === 'first-second-water'
 && debug.hotbarSlotKind === 'water'
 && (debug.starterMosslingSeedCount ?? 0) === (restocked.starterMosslingSeedCount ?? 0) - 1,
 );
 ...
 page,
 'Space',
 'second-round crop watered and first loop completed',
 (debug) => debug.frontTileWateredToday === true
 && debug.onboardingObjectiveId === 'first-loop-complete',
 );
