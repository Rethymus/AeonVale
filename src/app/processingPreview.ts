...
export function dryingProcessingPanelPreview(choice: { itemId: string; count: number; quality?: CropQuality }, content: ContentRegistry): ProcessingPanelPreview {
...
export function processingPositionRequiredToastPresentation(mode: 'drying' | 'sealing' | 'furnace'): ProcessingToastPresentation {
  return {
    message: mode === 'drying' ? '需站在晾晒架旁加工' : mode === 'sealing' ? '需站在封藏柜旁加工' : '需站在炼符炉旁加工',
    assetId: processingFacilityAssetId(mode)
  };
}

export function processingRecipeUnavailableToastPresentation(mode: 'furnace'): ProcessingToastPresentation {
  return {
