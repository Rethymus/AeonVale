...
function prepareTribulation(state: GameState, bot: BotPolicy, assistance: SyntheticAssistance, ctx: Parameters<typeof applyPill>[2]): void {
 ...
 state.player.hp = bot.recoveryProfile === 'inherit-limited'
 ? Math.min(state.player.maxHp, state.player.hp + Math.round(state.player.maxHp * bot.limitedRecoveryRatio))
 : state.player.maxHp;
