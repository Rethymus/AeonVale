...
function dryingHerbKinds(state: GameState): number {
 return Object.entries(state.player.inventory)
 .filter(([itemId, slot]) => itemId.startsWith('herb.') && (slot?.count ?? 0) > 0)
 .length;
}
...
function npcBrowseNextStepLine(
 npc: { id: string; affection: number },
 schedule: NpcDailySchedule | null,
 quest: NpcQuestStatus | null,
): string {
...
export function farmActionMenuPreview(kind: FarmActionKind, state: GameState, ctx: SimContext): ActionPanelPreview {
 ...
 const statusLine = state.facilities.size === 0
 ? '身旁暂无已建设施，可先去建造铺开经营位'
 : ready > 0
 ? '优先巡看已完工设施，把这一轮产物收住'
 : '当前暂无待收产物，先等加工完成或继续安排农务';
 return {
 title: '设施收取',
...
export function farmActionMenuToastPresentation(
 kind: FarmActionKind,
 indexLabel: string,
 confirmHint: string,
 state: GameState,
 ctx: SimContext,
): ActionMenuToastPresentation {
...
export function npcActionMenuPreview(
 mode: 'browse' | 'gift' | 'quest',
...
export function npcActionMenuToastPresentation(
 mode: 'browse' | 'gift' | 'quest',
 indexLabel: string,
 confirmHint: string,
 currentNpcId?: string | null,
...
export function npcBrowsePanelPreview(
 npc: { id: string; displayName: string; role: string } & RelationshipState,
 schedule: NpcDailySchedule | null,
 quest: NpcQuestStatus | null,
 giftName: string | null = null,
): ActionPanelPreview {
 const lines = [
 `${npc.role}｜好感 ${npc.affection}/1000`,
 schedule ? `${schedule.location}｜${schedule.activity}` : '今日行踪未明',
 ];
...
export function npcBrowseToastPresentation(
 npc: { id: string; displayName: string; role: string } & RelationshipState,
 schedule: NpcDailySchedule | null,
 quest: NpcQuestStatus | null,
 indexLabel: string,
 giftName: string | null = null,
): NpcPanelToastPresentation {
...
export function npcGiftPanelPreview(
 npc: { id: string; displayName: string },
 ...
 title: npc.displayName,
 details: giftName
 ? `当前最适礼物：${giftName}\n赠予 ${npc.displayName}\n${birthday ? '今日生辰｜好感收益翻倍' : '背包已备好｜可直接赠礼'}`
 : `赠予 ${npc.displayName}\n暂无合适礼物\n${birthday ? '今日生辰｜若补到礼物可翻倍' : '建议先去集市、药圃或仓库补货'}`,
 assetId: previewNpcPortraitAssetId(npc.id),
 };
}

export function npcGiftToastPresentation(
 npc: { id: string; displayName: string },
 giftName: string | null,
 birthday: boolean,
 indexLabel: string,
...
export function npcGiftResultToastPresentation(
 npc: { id: string; displayName: string },
 ...
 const successAssetId = giftItemId
 ? itemIconAssetId(giftItemId, content) ?? previewNpcPortraitAssetId(npc.id)
 : previewNpcPortraitAssetId(npc.id);
 return {
 message: outcome === 'success'
 ? `赠予${npc.displayName}${giftName ?? '礼物'}，好感 +${affectionGain ?? 0}${birthday ? '（生辰）' : ''}`
 : `${npc.displayName}：${giftName ? '今日已赠或物品不足' : '没有合适礼物'}`,
 assetId: outcome === 'success' ? successAssetId : previewNpcPortraitAssetId(npc.id),
 };
}

export function npcQuestPanelPreview(
...
const rewardAssetId = quest.reward.itemId
 ? itemIconAssetId(quest.reward.itemId, content) ?? previewNpcPortraitAssetId(quest.npcId)
 : previewNpcPortraitAssetId(quest.npcId);
...
export function npcQuestToastPresentation(
 npc: { displayName: string },
 quest: NpcQuestStatus | null,
 indexLabel: string,
 confirmHint: string,
...
export function npcQuestResultToastPresentation(
 npc: { displayName: string },
 quest: NpcQuestStatus | null,
 outcome: 'advance' | 'complete' | 'failure' | 'missing',
 content?: ContentRegistry,
