...
const SUMMARY_HINT_ROUTE_RULES: ReadonlyArray<SummaryHintRouteRule> = [
 ...
 exact: [
 '先去节日会场看看今日摊位，再决定要不要参与会场试礼。',
 '灵芽节已开，先去会场转一圈再安排今日农务。',
 ],
 ...
 exact: [
 '去阵坊核对阵核和符炉，再决定补哪一块控场能力。',
 '去阵器棚核对阵核和符炉，再决定补哪一块控场能力。',
 ],
...
function shippingLineLabel(
 ...
 const demandTag = line.demand?.source
 ? `〔${line.demand.source === 'special-order' ? '订单热需' : '委托热需'}+${line.demand.priceBonus ?? 0}〕`
 : '';
...
export function daySummaryMessage(
 day: number,
...
export function daySummaryPresentation(
 day: number,
 events: readonly GameEvent[],
 content: ContentRegistry,
 breakthroughReady: boolean,
