...
export function beastHuntResultToastPresentation(
 events: readonly GameEvent[],
 content: ContentRegistry,
): ExplorationToastPresentation {
 ...
 const rewardAssetId = coreEv
 ? itemIconAssetId(((coreEv.payload as { itemId?: string } | undefined)?.itemId) ?? '', content)
 : seedItemId
 ? itemIconAssetId(seedItemId, content)
 : undefined;
...
export function explorationResultToastPresentation(
 site: ExploreSite,
 grants: readonly ExplorationGrantLike[],
 content: ContentRegistry,
): ExplorationToastPresentation {
...
export function tribulationBlockedToastPresentation(
 kind: 'purple-omen' | 'body-not-ready',
 options?: { daysLeft?: number; currentFoundation?: number; requiredFoundation?: number },
): ExplorationToastPresentation {
 if (kind === 'purple-omen') {
...
const current = Math.floor((options?.currentFoundation ?? 0) / 1000);
 ...
 return {
 message: required > 0
 ? `体魄根基未满，还差 ${missing}｜先收灵草、炼丹或修行再引劫`
 : '体魄根基未满｜先收灵草、炼丹或修行再引劫',
 assetId: 'loc.farmstead',
 };
}

export function tribulationResultToastPresentation(
 kind: 'death' | 'breakthrough' | 'survived',
 options?: { stage?: number; temperingGain?: number },
): ExplorationToastPresentation {
 switch (kind) {
 case 'death':
 return {
...
export function tribulationEndingToastPresentation(
 kind: 'ascension' | 'death' | 'stay-in-world',
...
export function ruinDelveToastPresentation(
 result: RuinDelveToastResult,
 content: ContentRegistry,
): ExplorationToastPresentation {
 const grants = result.grants ?? [];
 const names = describeGrants(grants, content);
 const chapterText = result.chapterTitle
 ? `｜${result.chapterTitle} ${result.chapterProgress ?? '?'}${result.chapterReadyToClaim ? ' 可领' : ''}`
 : '';
 return {
