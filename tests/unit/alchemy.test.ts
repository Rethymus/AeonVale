...
describe('炼丹 sim ', () => {
  ...
  it('净毒丹方（露根草×2）出净毒丹', () => {
    const { state, ctx } = setup();
    const res = resolveBrew(state, { materials: [{ herbId: 'herb.dewroot', qty: 2 }], avgHeatMilli: 32_000 }, ctx);
  ...
  it('相须配伍（寒潭莲+露根草 同寒）提升 quality', () => {
    const { state, ctx } = setup();
    // 单寒潭莲 vs 寒潭莲+露根草(相须)：后者药性更集中、quality 更高（净毒丹方需露根×2，这里测纯聚合倾向）
