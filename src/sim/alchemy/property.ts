/**
 * 药性向量数学（docs/06 §2 / docs/14 §9）。
 * 内部四轴 [cold,hot,warm,neutral]（毫点）；玩家面投影为寒热轴 hot−cold（docs/20 R6）。
 * 炸炉主轴 conflict = |hot − cold|（docs/14 §9.3）。
 */
...
/** 平衡度评分（docs/14 §9.2）：clamp(1 − L1距离/norm, 0, 1)。 */
...
/** 由药性派生"自然理想火候"（寒草喜低温，热草喜高温，docs/14 §9.1 extraction 锚点）。返回毫点 0..100000。 */
...
/**
 * 火候-材料提取系数（docs/14 §9.1）。
 * 简化为线性衰减：在 naturalHeat 处峰值 1.0，±30(毫点×1000) 外归零。
 * （首版线性；后续可换 sin 拱形，docs/06 §4.3）
 */
