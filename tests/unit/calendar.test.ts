...
describe('日历节奏', () => {
  it('按季节列出内容表节日和 NPC 生辰，并按日期排序', () => {
    const { ctx } = setup();
    expect(calendarEntriesForSeason(ctx, 'spring').map(entry => `${entry.day}:${entry.title}`)).toEqual(['14:灵芽节', '18:游方散修生辰']);
  ...
  it('当日天象未在日历中重复出现时追加为活跃事项', () => {
    ...
    expect(calendarEntriesForDay(state, ctx)).toMatchObject([{ kind: 'celestial-active', id: 'event.qi-tide', title: '灵气潮汐', description: '剩余3日' }]);
  });
