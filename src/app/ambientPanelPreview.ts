...
function formatLocationPreviewDetails(
 location: LocationStatus,
 service: LocationServiceOption | null,
 ...
 ? `去处：${service.label} -> ${service.commandLabel}`
 : '去处：今日先回这里整顿农务与行程';
 const encounterLine = locationEncounterLine(location, encounterNames);
 return [location.description, signalLine, focusReason ? `现在来：${focusReason}` : '', actionLine ?? '', serviceLine, encounterLine]
 .filter((line) => line.length > 0)
 .join('\n');
}

function locationEncounterLine(location: LocationStatus, encounterNames: readonly string[]): string {
 const encounterLine = encounterNames.length > 0
 ? `人物：${encounterNames.join('、')}`
 : (location.npcs.length > 0 ? `人物：${location.npcs.join('、')}` : '人物：今日先按眼前去处推进');
...
function farmsteadFallbackSelection(
 state: GameState,
 baseLocation: LocationStatus,
): { location: LocationStatus; service: LocationServiceOption | null; actionLocationId: LocationId } {
...
if (readyFacilityKind === 'drying-rack') {
 const service = farmsteadServices.find((entry) => entry.command === 'show-processing')
 ?? getLocationServiceOptions(state, 'drying-yard').find((entry) => entry.command === 'show-processing')
 ?? null;
 return { location: baseLocation, service, actionLocationId: 'farmstead' };
 }

if (readyFacilityKind === 'talisman-furnace') {
 const service = farmsteadServices.find((entry) => entry.command === 'show-arrays')
 ?? getLocationServiceOptions(state, 'array-shed').find((entry) => entry.command === 'show-arrays')
 ?? null;
 return { location: baseLocation, service, actionLocationId: 'farmstead' };
 }
...
const previewLocationId = farmsteadFocusPreviewLocationId(farmsteadFocus);
 ...
 const previewLocation = locationStatusById(state, previewLocationId)
 ?? ({ ...LOCATION_CATALOG.find((entry) => entry.id === previewLocationId)!, active: true, npcs: [], serviceLabels: [], closedServiceLabels: [] });
 const service = getLocationServiceOptions(state, previewLocationId)[0]
 ?? farmsteadServices.find((entry) => entry.command === 'show-farm-work')
 ?? farmsteadServices[0]
 ?? null;
...
if (readyForBreakthrough(state, DEFAULT_BALANCE)) {
 const location = activeLocations.find((entry) => entry.id === 'array-shed')
 ?? ({ ...LOCATION_CATALOG.find((entry) => entry.id === 'array-shed')!, active: true, npcs: [], serviceLabels: [], closedServiceLabels: [] });
 ...
 const actionLine = formatLocationActionSignalLine(state, 'array-shed', '要务')
 ?? `要务：${tribulationPrepStatusLine(state)}`;
...
return {
 ...
 signalLine,
 `现在来：${tribulationPrepFocusReason(state)}`,
 actionLine,
 '去处：阵法布设 -> 查看阵法',
 '人物：今日先按眼前去处推进',
...
const preferred = getPreferredLocationSelection(state);
 ...
 const location = (objectiveId === 'first-loop-complete' && prioritySignal
 ? prioritySignal.location
 : null) ?? (preferred
 ? activeLocations.find((entry) => entry.id === preferred.locationId)
 : null) ?? prioritySignal?.location ?? activeLocations[0] ?? null;
 if (!location) return null;

const baseServices = getLocationServiceOptions(state, location.id);
 const baseService = (preferred && preferred.locationId === location.id
 ? baseServices.find((entry) => entry.command === preferred.command)
 : null) ?? baseServices[0] ?? null;

const fallbackSelection = !preferred && location.id === 'farmstead'
 ? farmsteadFallbackSelection(state, location)
 : { location, service: baseService, actionLocationId: location.id };
...
const previewLocation = fallbackSelection.location;
 ...
 const signalLine = formatLocationNpcSignalLine(collectLocationNpcSignals(state, encounters), '去向');
 const routeFocusReason = locationPreviewFocusReason(
 state,
 objectiveId,
 previewLocation.id,
 ...
 const focusReason = fallbackSelection.actionLocationId === 'farmstead'
 ? (objectiveId === 'first-loop-complete' && service?.command === 'show-processing'
 ? routeFocusReason
 : getFarmsteadFocus(state).locationReason)
 : routeFocusReason;
...
export function ambientPanelPreview(
 state: GameState,
 content: ContentRegistry,
 inventoryVisible: boolean,
): AmbientPanelPreview | null {
