...
simulateDay(withArray, { actions: [{ kind: 'water', at: { x: 3, y: 3 } }, { kind: 'channel-qi', at: { x: 3, y: 3 } }] }, withArrayCtx);
 simulateDay(baseline, { actions: [{ kind: 'water', at: { x: 3, y: 3 } }, { kind: 'channel-qi', at: { x: 3, y: 3 } }] }, baselineCtx);
...
it('引雷阵覆盖的成熟金属性灵草过熟衰减减半（诱饵田回报）', () => {
 ...
 tile.tilled = true;
 current.crops.set(tile.id, {
 id: 1, defId: 'herb.metalpine', tileId: tile.id,
 growth: threshold, health: 100 * MILLI, stage: 'mature', plantedDay: 1,
 property: { cold: 0, hot: 0, warm: 0, neutral: 0 }, tempered: false,
 });
 tile.cropId = 1;
 }
 placeArray(withArray, 'array.lightning-rod', 3, 3, withArrayCtx, { free: true });
...
expect(state.events.find((e) => e.type === 'harvest')?.payload).toMatchObject({
 rodTempered: true, rodQualityScoreBonus: 0.12, rodYieldBonus: 1,
 });
 });
