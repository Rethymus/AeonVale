...
async function clearIntroDialogue(page: Page): Promise<void> {
 ...
 const alreadyCleared = await page.evaluate(({ key, expectedFlag }: { key: string; expectedFlag: string }) => {
...
await page.waitForFunction((id) => {
 ...
 await page.keyboard.press('Enter');
 await page.waitForFunction(({ key, flag }: { key: string; flag: string }) => {
 const raw = window.localStorage.getItem(key);
 if (!raw) return false;
 const parsed = JSON.parse(raw) as { state?: { player?: { flags?: string[] } } };
...
type SaveFixtureMode =
 | 'build-panel'
 | 'furnace-build-panel'
 | 'sealing-processing-panel'
 | 'furnace-processing-panel'
 | 'festival-panel'
 | 'shop-panel'
 | 'shop-panel-stable'
 | 'shop-panel-trade'
 | 'storage-panel'
 | 'shipping-panel'
 | 'facility-collect-panel'
 | 'hotbar-primary'
 | 'end-day-ready'
 | 'exploration-panel'
 | 'first-restock-ready'
 | 'second-sow-raw-front'
 | 'ascension-choice'
 | 'ascension-pill'
 | 'post-ascension-commission'
 | 'post-ascension-tea-shed'
 | 'post-ascension-greenhouse'
 | 'post-ascension-quick-access'
 | 'post-ascension-greenhouse-upgrade';
...
async function installSave(page: Page, mode: SaveFixtureMode): Promise<void> {
...
await page.keyboard.press('O');
 ...
 await page.waitForTimeout(40);
 await page.keyboard.press('.');
 await page.waitForFunction(({ key, previousSpiritStones, previousFrostmarrowSeeds }) => {
 const raw = window.localStorage.getItem(key);
 if (!raw) return false;
 const parsed = JSON.parse(raw) as SaveSnapshot;
 return (parsed.state?.player?.inventory?.['item.spirit-stone']?.count ?? 0) < previousSpiritStones
 && (parsed.state?.player?.inventory?.['seed.frostmarrow']?.count ?? 0) > previousFrostmarrowSeeds;
...
await page.keyboard.press('Enter');
 await page.waitForFunction(({ key, previousStamina }) => {
 const raw = window.localStorage.getItem(key);
 if (!raw) return false;
 const parsed = JSON.parse(raw) as SaveSnapshot;
...
await page.locator('canvas').click;
 ...
 await page.locator('canvas').click;
 await page.waitForFunction(({ key, previousCount }) => {
 const raw = window.localStorage.getItem(key);
 if (!raw) return false;
 const parsed = JSON.parse(raw) as SaveSnapshot;
...
const debug = await page.evaluate(() => (window as typeof window & {
...
await preselectMarketShop(page);
 await confirmSelectedService(page, 'shop');
 await page.keyboard.press('Enter');
 await page.waitForFunction(({ key, previousSeedCount, previousSpiritStones }) => {
 const raw = window.localStorage.getItem(key);
 if (!raw) return false;
 const parsed = JSON.parse(raw) as SaveSnapshot;
 return (parsed.state?.player?.inventory?.['seed.mossling']?.count ?? 0) > previousSeedCount
 && (parsed.state?.player?.inventory?.['item.spirit-stone']?.count ?? 0) < previousSpiritStones;
...
await preselectMarketShop(page);
 await confirmSelectedService(page, 'shop');
 await page.keyboard.press('Enter');
 await page.waitForFunction(({ key, previousSeedCount }) => {
 const raw = window.localStorage.getItem(key);
 if (!raw) return false;
 const parsed = JSON.parse(raw) as SaveSnapshot;
 return (parsed.state?.player?.inventory?.['seed.mossling']?.count ?? 0) > previousSeedCount;
...
await page.keyboard.press('Space');
 ...
 return (targetTile?.cropId != null)
 && (parsed.state?.player?.inventory?.['seed.mossling']?.count ?? 0) === previousSeedCount
 && flags.includes('onboarding-first-second-sow');
 }, { key: SAVE_KEY, previousSeedCount: beforeSeedCount });
...
await clearBlockingDialogue(page);
 await page.keyboard.press('Enter');
 await page.waitForFunction(({ key, previousDay }) => {
 const raw = window.localStorage.getItem(key);
 if (!raw) return false;
 const parsed = JSON.parse(raw) as SaveSnapshot;
...
const afterDismiss = await readSave(page);
 ...
 const debugAfterDismiss = await page.evaluate(() => (window as typeof window & {
 __AEON_DEBUG__?: { interactionPanelKind?: string };
 }).__AEON_DEBUG__);
...
test('ascension choice 1 clears save after ascend-away ending', async ({ page }) => {
...
await clearIntroDialogue(page);
 await page.keyboard.press('1');
 await page.waitForFunction((key) => window.localStorage.getItem(key) === null, SAVE_KEY);
...
await clearIntroDialogue(page);
 ...
 if (!raw) return false;
 const parsed = JSON.parse(raw) as SaveSnapshot;
 return parsed.state?.postAscension?.mode === 'stayed-in-world'
 && parsed.state?.postAscension?.victoryRecorded === true
 && parsed.state?.gameOver === false;
 }, SAVE_KEY);
...
await clearIntroDialogue(page);
 ...
 if (!raw) return false;
 const parsed = JSON.parse(raw) as SaveSnapshot;
 return (parsed.state?.player?.inventory?.['item.spirit-stone']?.count ?? 0) === 16
 && (parsed.state?.player?.inventory?.['item.beast-core']?.count ?? 0) === 0
 && (parsed.state?.flags?.includes('commission.6.commission.human-ward-patrol') ?? false);
 }, SAVE_KEY);
...
for (let i = 0; i < 9; i += 1) {
 ...
 if (!raw) return false;
 const parsed = JSON.parse(raw) as SaveSnapshot;
 return (parsed.state?.flags?.includes('greenhouse-tended.1') ?? false)
 && (parsed.state?.player?.inventory?.['seed.dewroot']?.count ?? 0) >= 2;
 }, SAVE_KEY);
...
await page.keyboard.press('Alt+E');
 ...
 const parsed = JSON.parse(raw) as SaveSnapshot;
 return (parsed.state?.flags?.includes('commission.6.commission.human-ward-patrol') ?? false)
 && (parsed.state?.flags?.includes('tea-shed-visit.6') ?? false)
 && (parsed.state?.flags?.includes('greenhouse-tended.6') ?? false);
 }, SAVE_KEY);
...
await page.keyboard.press('Enter');
 ...
 if (!raw) return false;
 const parsed = JSON.parse(raw) as SaveSnapshot;
 return (parsed.state?.flags?.includes('upgrade.greenhouse-nursery-1') ?? false)
 && (parsed.state?.player?.inventory?.['item.spirit-stone']?.count ?? 0) === 0;
 }, SAVE_KEY);
