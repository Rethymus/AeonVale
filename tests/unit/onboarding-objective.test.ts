...
describe('onboarding objective helper', () => {
  ...
  it('blocks end-day during the few onboarding steps that would otherwise be easy to skip by mistake', () => {
    expect(onboardingEndDayWarning('first-ship')).toBe('先别过夜：把第一株灵草投进出货箱。｜动线：收下成熟灵草后，顺手投进出货箱。');
  ...
  it('anchors first-session loop handoff toasts to the current onboarding subject', () => {
    ...
    expect(onboardingWelcomeToastPresentation(onboardingHelpText('first-loop-complete'))).toEqual({
      message: `修仙农庄开局：灵草换灵石，灵石撑备劫。${onboardingObjectiveText('first-loop-complete')}`,
      assetId: 'loc.herb-plot'
    });
  });
