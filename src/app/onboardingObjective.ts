...
const ONBOARDING_OBJECTIVE_IDS: readonly OnboardingObjectiveId[] = ['first-till', 'first-sow', 'first-water', 'first-harvest', 'first-ship', 'first-sleep', 'first-market-restock', 'first-second-sow', 'first-second-water', 'first-loop-complete'];
...
const END_DAY_BLOCKED_OBJECTIVES = new Set<OnboardingObjectiveId>(['first-ship', 'first-market-restock', 'first-second-sow', 'first-second-water']);
...
export function onboardingWelcomeToastPresentation(helpText: string): OnboardingToastPresentation {
  ...
  const objectiveLine = primaryObjectiveLine(helpText);
  return {
    message: objectiveLine ? `修仙农庄开局：灵草换灵石，灵石撑备劫。${objectiveLine}` : '修仙农庄开局：灵草换灵石，灵石撑备劫。',
    assetId: onboardingObjectiveAssetId(objectiveId)
  };
}
