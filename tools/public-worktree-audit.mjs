...
export function classifyPublicCandidateGroup(path) {
 ...
 || path === 'tools/public-tree-rules.mjs'
 || path === 'tools/prepare-public-tree.mjs'
 || path === 'tools/public-content-audit.mjs'
 || path === 'tools/public-content-audit.d.mts'
 || path === 'tools/public-readiness-check.mjs'
 || path === 'tools/publication-check.mjs'
 || path === 'tools/public-dist-check.mjs'
 || path === 'tools/public-worktree-audit.mjs'
 || path === 'tools/public-worktree-audit.d.mts'
 || path === 'tools/portfolio-mvp-preflight.mjs'
 ) {
...
if (path.startsWith('src/sim/') || path.startsWith('src/content/')) return 'gameplay-sim';
 ...
 || path.startsWith('tests/replay/')
 || path.startsWith('tests/headless/')
 ) {
...
export function shouldFailPublicWorktreeAudit(report, options = {}) {
 return Boolean(
 (options.failOnPublicCandidates && report.counts['public-candidate'] > 0)
 || (options.failOnSecretRisk && report.counts['secret-risk'] > 0),
 );
}

function formatList(items) {
 if (!items.length) return ' - none';
 return items.slice(0, 40).map((item) => ` - ${item}`).join('\n');
}
...
function formatReport(report) {
 const lines = [
 'Public worktree audit',
 '',
 'Changed path classes:',
 ...classes.map((name) => ` ${name}: ${report.counts[name]}`),
 '',
 'Dirty public candidates included by public-tree rules:',
 formatList(report.byClass['public-candidate']),
 '',
 'Dirty public candidates by release bucket:',
 formatCandidateGroups(report),
 '',
 'Private design docs, local Agent state, generated output, env files, and sourcemaps are excluded by public-tree rules.',
 ];
