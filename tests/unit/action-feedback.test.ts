...
it('detects watering from same-day mark or moisture increase', () => {
 expect(deriveFarmActionOutcome(
 'water',
 [makeTile({ cropId: 1, moisture: 10_000 })],
 [makeTile({ cropId: 1, wateredToday: true, moisture: 40_000 })],
 )).toEqual({
 succeeded: true,
 affectedTiles: [{ x: 0, y: 0 }],
 });
...
it('detects sow and fertilize only when the tile state actually advances', () => {
 expect(deriveFarmActionOutcome(
 'sow',
 [makeTile({ tilled: true, cropId: null })],
 [makeTile({ tilled: true, cropId: 8 })],
 )).toEqual({
 succeeded: true,
 affectedTiles: [{ x: 0, y: 0 }],
 });
...
expect(deriveFarmActionOutcome(
 'fertilize',
 [makeTile({ tilled: true, fertility: 20_000, qiDensity: 10_000 })],
 [makeTile({ tilled: true, fertility: 45_000, qiDensity: 25_000 })],
 )).toEqual({
 succeeded: true,
 affectedTiles: [{ x: 0, y: 0 }],
 });
...
it('detects channel-qi only when a planted tile gains same-day qi care', () => {
 ...
 'channel-qi',
 [makeTile({ cropId: 4 })],
 [makeTile({ cropId: 4, channeledToday: true })],
 )).toEqual({
 succeeded: true,
 affectedTiles: [{ x: 0, y: 0 }],
 });
...
it('anchors sow success to the active farm root thread', () => {
 ...
 expect(sowSuccessToastPresentation({
...
it('anchors array placement success and failure feedback to concrete array facility art', () => {
 ...
 expect(arrayPlacementToastPresentation('lightning-rod', {
 placed: false,
 reason: '引雷阵需金属性灵草作阵眼',
 costText: '阵核x1、灵石x4',
 })).toEqual({
 ...
 expect(arrayPlacementToastPresentation('insulation', {
 placed: false,
 costText: '阵核x1、灵石x2',
 })).toEqual({
