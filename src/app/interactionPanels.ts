export type InteractionPanelState =
 ...
 | { kind: 'build' }
 | { kind: 'upgrade' }
 | { kind: 'npc'; mode: 'browse' | 'gift' | 'quest' }
 | { kind: 'festival' }
 | { kind: 'shop'; festival: boolean }
 | { kind: 'trade' }
 | { kind: 'commission' }
 | { kind: 'tea-shed' }
 | { kind: 'greenhouse' }
 | { kind: 'storage'; mode: 'deposit' | 'withdraw' }
 | { kind: 'shipping'; mode: 'normal' | 'quality' }
 | { kind: 'processing'; mode: 'drying' | 'sealing' | 'furnace' }
 | { kind: 'facility-collect' };
...
export const FARM_ACTION_ORDER: readonly FarmActionKind[] = [
 'build',
 'facility-collect',
 'storage-deposit',
 'storage-withdraw',
