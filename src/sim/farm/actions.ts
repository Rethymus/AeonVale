...
function canFitPotentialHarvestRewards(
 state: GameState,
 ctx: SimContext,
 crop: CropInstance,
 tileId: number,
...
function crossTiles(state: GameState, x: number, y: number, maxCount: number) {
 ...
 return offsets.slice(0, Math.max(1, Math.min(maxCount, offsets.length))).map((d) => tileAt(state, x + d.x, y + d.y)).filter((t): t is NonNullable<ReturnType<typeof tileAt>> => Boolean(t));
}
