...
describe('日终摘要反馈', () => {
  ...
  it('首轮出货结清时把灵石用途接回补种、炉料与备劫循环', () => {
    ...
    expect(daySummaryMessage(2, events, reg, false, ['当前目标：去山谷集市补几颗种子，把第二轮药材接上。', '意义：补种把一次收获变成稳定经营，后续才有炼丹库存。', '操作：按 Shift+Tab 打开地点目录，选集市服务后确认补种。'].join('\n'))).toBe('首轮出货结清｜出货结算：得灵石 ×8｜凡间青苔×2（8）｜用途：灵石先补种子、炉料与备劫消耗。｜下一步：去山谷集市补几颗种子，把第二轮药材接上。｜接力：按 Shift+Tab 打开地点目录，选集市服务后确认补种。');
  });

  it('在日终摘要已包含下一步时，不再重复拼接额外的 onboarding 推进 toast', () => {
    expect(composeEndDayToastMessage('第 4 日｜下一步：去山谷集市补几颗种子，把第二轮药材接上。｜接力：按 Shift+Tab 打开地点目录，选集市服务后确认补种。', '下一步：回到农庄，把新买到的种子立刻播回田里。｜面向已翻土地按 Z 补种。')).toBe('第 4 日｜下一步：去山谷集市补几颗种子，把第二轮药材接上。｜接力：按 Shift+Tab 打开地点目录，选集市服务后确认补种。');
  });

  it('在日终摘要没有下一步时，仍会补上目标推进 toast', () => {
    expect(composeEndDayToastMessage('第 4 日', '下一步：回到农庄，把新买到的种子立刻播回田里。｜面向已翻土地按 Z 补种。')).toBe('第 4 日\n下一步：回到农庄，把新买到的种子立刻播回田里。｜面向已翻土地按 Z 补种。');
  });
