...
#### 3.4.3 `雷兽孵化 Lightning Beast Hatching`（危 + 机）

**叙事**：之前紫雷劈中处留下的"雷种"孵化出雷兽幼崽。

**因果链**：
1. **Preview (1 天)**：旧雷击点异动。
2. **Active (3 天)**：雷兽幼崽出现，会啃食金属属性灵草（破坏避雷阵布局！）。但若喂食培育（用特定丹药），成年后可驯化为"活体避雷针"（移动阵眼）。
3. **Climax**：第 3 天决定——杀（获 `雷兽内丹 BeastCore`，强力炼丹材料）、放（无奖励但 -HeavenlyScrutiny）、驯化（长线投资）。
4. **Aftermath**：驯化兽成为永久阵法资产。

**应对**：权衡短期材料 vs 长期阵法助力。

**数值钩子**：`BeastHerbPerDay`(2 株), `TamingPillCost`(3 颗特定丹), `TamedBeastArrayPower`(= 1.5 株成熟金属性草).

#### 3.4.4 `灵脉异动 Spirit Vein Shift`（机 + 危）
...
#### 3.4.5 `天谴余波 Heavenly Wrath Aftershock`（危，罕见）

**叙事**：远处有大能逆天失败，天道余波扫荡。

**因果链**：
1. **Preview (0 天)**：天空血红，诡异寂静。
2. **Active (1 天)**：天劫倒计时**立即注入** `-InjectSeconds`（见 `05-...md` §1.3，默认 -30 s），强制提前渡劫！同时全场灵草药性暂时紊乱（±1 偏移）。随机 1 格农田被"天罚"摧毁。
3. **Resolution**：余波过。
4. **Aftermath**：被毁格变"天罚之地"，N 天内不可种植，但可挖掘获 `天罚之灰 DivineAsh`（极稀有炼丹材料，用于走火丹）。

**应对**：几乎无法阻止，只能即时调整——提前渡劫要求玩家始终保持"准渡劫"状态（阵法常备、丹药常储）。
...
- **天骄降世 + 雷兽孵化**：灵气潮汐加速雷兽成长（更早 climax）。
...
| `BeastHerbPerDay` | 2 | 雷兽 | 啃食量 |
| `TamingPillCost` | 3 | 雷兽 | 驯化丹耗 |
| `TamedBeastArrayPower` | 1.5 草 | 雷兽 | 驯化兽阵力 |
...
- **Q1**：`天谴余波` 这种"强制提前渡劫"是否过于惩罚？是否给玩家一个"以 HP 换推迟"的赎买选项？本文保留惩罚以维持紧张感。
