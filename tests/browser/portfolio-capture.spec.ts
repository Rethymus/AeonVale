...
async function capturePortfolioScreenshot(page: Page, path: string): Promise<void> {
...
function buildPortfolioMvpEvidence(
 ...
 'test-results/portfolio/02-location-routing.png',
 'test-results/portfolio/03-farm-actions.png',
...
test('captures deterministic review screenshots for public demo validation', async ({ page }) => {
 ...
 await capturePortfolioScreenshot(page, 'test-results/portfolio/01-farm-loop.png');
...
test('captures a mobile-safe first screen for GitHub Pages demo review', async ({ page }) => {
 ...
 await expectPortfolioFarmLoopState(page);
 await capturePortfolioScreenshot(page, 'test-results/portfolio/04-mobile-farm-loop.png');
});
