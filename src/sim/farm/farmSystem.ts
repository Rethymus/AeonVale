...
export function applyFarmDayEnd(state: GameState, ctx: SimContext, growthMod = 1, qiMod = 1): void {
 ...
 const tileQiMod = lightningRodCovered && herb.metalAttract > 0
 ? qiMod + LIGHTNING_ROD_ARRAY_QI_REGEN_BONUS + (resonance?.qiRegenBonus ?? 0)
 : qiMod;

if (crop.growth < herb.growthThreshold) {
 const delta = growthPerDay(
 herb,
 tile,
 state.season,
 params,
 greenhouseProtected ? growthMod * greenhouseGrowthMod : growthMod,
 greenhouseProtected,
 );
 ...
 const overripeDecayMilli = (lightningRodCovered && herb.metalAttract > 0)
 ? Math.round(params.growth.overripeDecay * 0.5 * MILLI)
 : params.growth.overripeDecay * MILLI;
...
// 2. 空地块灵气自然再生
 ...
 const tileQiMod = lightningRodCovered
 ? qiMod + LIGHTNING_ROD_ARRAY_QI_REGEN_BONUS
 : insulatedTile ? qiMod + INSULATION_ARRAY_QI_REGEN_BONUS : qiMod;
 if (tile.cropId === null) {
