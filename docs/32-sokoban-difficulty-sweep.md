...
**第二次纵向采样（2026-09-17 晚，同命令）**：LCP 中位 **6124ms**、
FCP 1428ms、CLS 0、TBT 0、字体发现 417ms。两窗口方差带：

| 指标 | 窗口一 | 窗口二 | 方差带 |
| ---- | ------ | ------ | ------ |
| LCP 中位 | 5740ms | 6124ms | **5740–6124（±3.3%）**——稳定，载荷带宽下限口径成立 |
| FCP 中位 | 3720ms | 1428ms | 1428–3720（**2.6× 摆动**）——FCP 对连接爬坡最敏感，仅看趋势 |
| CLS / TBT | 0 / 0 | 0 / 0 | 恒 0，稳定 |
| 字体发现 | 356ms | 417ms | 356–417ms，preload 稳定生效 |

后续移动性能回归：重跑同命令对比本表；LCP 判定继续按「载荷带宽下限」
口径解读，超出 5740–6124 带即查资产/构建变更；架构级优化（代码分割/
延迟 Pixi 启动/字体分片）仍以真实用户网络分布数据（docs/30 §六）为启动条件。


### 24.7 桌面维度正式纵向基线（2026-09-17，pnpm perf:audit --loads=3）

与 §24.6.1 移动基线对称，桌面维度回归以固化工具输出为准（线上站点、
全新 context 冷载、无节流、中位数）。首轮：

| 指标 | 中位 | 三次分列 | 阈值判定 |
| ---- | ---- | -------- | -------- |
| LCP | **492ms** | 1092 / 492 / 484 | ✓ ≤2500 |
| FCP | 484ms | 884 / 348 / 484 | ✓ ≤1800 |
| CLS | 0 | 全 0 | ✓ |
| TBT | 0ms | 全 0 | ✓（INP 代理） |
| 字体发现/完成 | 154 / 301ms | 448/819、150/257、154/301 | preload 并行化生效 |
| 载荷 | JS 402KB + 其他 423KB | — | 与 §24.4 优化后一致 |

口径说明：跨时段绝对值受网络噪声支配（对照 §24.4 优化后会话 LCP 中位
1484ms 与 §24.6 期间某窗口 2996ms——载荷相同、网络不同）；回归判定以
**同窗口重跑对比本表**为准，跨窗口仅看趋势。桌面全指标处于 good 区间，
无优化工作项；移动维度结论见 §24.6/§24.6.1。

**第二次纵向采样（2026-09-17 晚，同命令）**：LCP 中位 **1052ms**、
FCP 864ms、CLS 0、TBT 0、字体发现 403ms。两窗口方差带：

| 指标 | 窗口一 | 窗口二 | 方差带 |
| ---- | ------ | ------ | ------ |
| LCP 中位 | 492ms | 1052ms | **492–1052（2.1×）**——无节流桌面 LCP 对网络窗口敏感，但两窗口均 ≤2500 全绿 |
| FCP 中位 | 484ms | 864ms | 484–864ms |
| CLS / TBT | 0 / 0 | 0 / 0 | 恒 0，稳定 |
| 字体发现 | 154ms | 403ms | 154–403ms |

**第三次本地窗口（2026-09-17，同命令）**：LCP 中位 **552ms**（分列
1152/552/524）、CLS 0、TBT 0——落带内，与窗口一（492ms）共同加固
下限证据：本地网络正常窗口的 LCP 地板约 500ms，1052ms 上限属慢窗口。

**第四次本地窗口（2026-09-22，修复后探针）**：LCP 中位 **468ms**
（分列 1364/440/468）、CLS 0、**真实 TBT 25–115ms**（中位 65——
探针修复后首个本地实测，与 §24.8 Lighthouse 模拟 60ms 同量级互证）。
四窗口证据（492/552/468/1052）确认：本地正常窗口 LCP 地板约 470–550ms，
带下限 492 无需调整；1052 属慢窗口尾部。

**第五窗口（2026-09-23 周三，修复后探针）**：LCP 中位 **1252ms**
（分列 2112/1216/1252）——**首次超带上限**（1052），真实 TBT 124–150ms。
归因：无应用层变更（同日 Lighthouse 交叉验证与 dispatch 带判定均正常），
属慢网络窗口（首载 TTFB 500ms）。**带值复审（5 窗口）**：desktop 本地
带上限 1052→**1300**（覆盖慢窗口尾部 1052/1252；地板 468–550 不变），
vertical-bands.json 已同步。后续以 1300 为回归判定线，再超即查应用层。

**第五窗口计划（待执行）**：2026-09-23（周三）至 09-25（周五）期间、与
第四窗口间隔 ≥1 天时执行 `pnpm perf:audit --loads=3`，LCP/TBT 中位按
第四窗口格式补记入本节。同日重采样无跨时间方差增量，不作数。

回归口径（2026-09-17 首次带维护评审后修订）：桌面无节流样本呈**环境
双簇**——本地住宅网络（492/552/1052ms 三窗口）与 CI 数据中心
（360/468ms，两次工作流采样均低于本地带下限）——故 vertical-bands.json 分设
`desktop`（本地带 492–1052）与 `desktop-ci`（runner 带 **320–620**，
观测簇 360–468 加工程余量）两剖面；工具按 `GITHUB_ACTIONS` 环境自动
选带。移动节流归一化环境（CI 样本 5812/5816 在带内），不分区。
带值维护：窗口增补或载荷变更时人工更新 vertical-bands.json。
**首次窗口增补评审（2026-09-17，4 个 CI 桌面窗口后）**：新增窗口
272/284ms 低于原 desktop-ci 下限 320——下限放宽至 **260**（观测簇
272–468 全覆盖，上限 620 不变、无样本逼近）；移动 5 窗口全带内不动。
验收自动化（automation-457ef6ba）已排期 2026-09-21 触发后执行 §24.7.2
清单，结果届时入链。

**定时触发核对（2026-09-17）**：`gh api .../actions/workflows` 确认
perf-vertical.yml 注册状态 **active**（cron `23 2 * * 1`，下次触发
2026-09-21）；工作流本体已三次 dispatch 实跑验证（含自动回提交）。
平台策略注记：仓库 60 天无提交活动会自动停用 schedule 触发——本仓库
高频提交，不适用。
...
**附：带判定的载荷数纪律**（移动剖面实测教训）：带值基于 3 载荷中位，
loads=1 试跑的 LCP 单值对窗口噪声敏感（实测 6492ms 出带 6124，而
loads=3 中位始终带内）——周度工作流的帧预算步（--loads=1）因此不带
--bands，LCP 带判定由 3 载荷步承担；单载荷记录仅作帧预算与诊断用途。
