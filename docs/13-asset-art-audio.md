...
| 灵草精灵（每品阶 1–9） | **手绘像素优先**（16×16 或 32×32） | 主题核心辨识，需要性格；开发期可先走程序化/占位 |
| 角色精灵（玩家、NPC、妖兽） | **手绘像素优先**（32×32 起步） | 叙事载体；开发期允许经授权、留痕、审核通过的占位批次 |
...
- **默认不直接进游戏**：AI 生成图风格不一致、版权边界弱、分辨率不齐。
- 仅作为「调色板」「构图」「笔触」的灵感来源，最终美术由人或像素工具（Aseprite）落地。
- **唯一例外**：结局静态 CG 可考虑 AI 生成 + 后期统一滤镜（保持风格一致）。
- **开发期例外**：若用户明确授权，且资源已完成尺寸/调色/内容/来源留痕审核，可作为占位角色精灵入库联调；正式发行仍以统一风格正式资源为目标。
...
| 角色（玩家 + NPC + 妖兽） | ~15（当前已入库 5 张首批占位） | 32×32 / 64×64 | ~150KB |
...
| 程序化 SFX | 自合成 | 无版权风险 |
| BGM 序列 | 自创 | 无版权风险 |
| 灵感参考图 | AI 生成 / 公开素材 | 默认**不进游戏**，仅作 mood board；开发期占位需单独授权并留痕 |

**禁令**：任何来源不明的资产禁止入库；`assets/manifest.json` 必须为每条资产记录 `license` 与 `source` 字段。AI 占位资产若被允许入库，必须额外记录模型/来源/用途并接受后续替换。
...
- 键位可重映射。
- 鼠标点击容差（点击阵眼附近也算命中）。
- 雷劫期可设置「时间减速 0.7×」（仅单机，不破坏排行榜——本游戏无在线排行）。
...
- `assets/manifest.json` 当前已登记 3 张结局 CG 与 5 张 `sprite.*` 角色精灵占位资产。
- 这些角色精灵属于开发期联调用占位批次，已通过仓库内完整性测试，不代表正式发行版风格已锁定。
- 与另一侧 `Claude Code` 的美术协作应优先补齐：更多 NPC、巡守兽变体、设施精灵、高频种子/灵草/丹药图标。
