...
interface Report {
  ...
  qualificationRule: 'point-estimate-in-band';
  strictWilsonCertificationDeferredTo: 'M6 human playtest proxy calibration';
  profile: Profile;
  seedStart: number;
  seeds: number;
  maxDays: number;
...
function parseOptions(args: string[]): Options {
  ...
  const defaults = profile === 'pr' ? { seeds: 64, seedStart: 1, maxDays: 2_000, failOn: 'structural' as const, repeatDeterminism: 8 } : profile === 'nightly' ? { seeds: 1_000, seedStart: 20_001, maxDays: 2_000, failOn: 'targets' as const, repeatDeterminism: 16 } : { seeds: 32, seedStart: 10_001, maxDays: 2_000, failOn: 'never' as const, repeatDeterminism: 4 };
...
function toText(report: Report): string {
  const lines = ['M5 assisted campaign proxy', 'Not a human campaign-duration certification.', `Duration unit: ${report.durationUnit}; humanHoursCertified: ${report.humanHoursCertified}`, `Qualification: point estimate in target band; strict Wilson certification deferred to M6 human playtest proxy calibration.`, `Profile ${report.profile}: ${report.seeds} seeds from ${report.seedStart}, horizon ${report.maxDays} game-days`, ''];
  ...
  for (const [bot, result] of Object.entries(report.targetResults)) {
    lines.push(` ${bot} target: ${result.status}; pointEstimateQualified=${result.pointEstimateQualified}; strictWilsonCertified=${result.strictWilsonCertified}`);
  }
...
function main(): void {
  ...
  const report: Report = {
    ...
    cohorts,
    structuralFailures: structuralFailures(cohorts),
    targetResults
  };
  console.log(options.format === 'json' ? JSON.stringify(report, null, 2) : toText(report));
  const targetFailed = Object.values(targetResults).some(result => !result.pointEstimateQualified);
