...
export const HOTBAR_SLOTS: readonly HotbarSlot[] = [
 ...
 { kind: 'harvest' },
 { kind: 'channel-qi' },
 { kind: 'seed', seedId: 'seed.mossling' },
 { kind: 'seed', seedId: 'seed.dewroot' },
 { kind: 'seed', seedId: 'seed.suncap' },
 { kind: 'seed', seedId: 'seed.stonegrain' },
 { kind: 'seed', seedId: 'seed.mistfern' },
 { kind: 'seed', seedId: 'seed.sunmoss' },
];
...
export function findNextHotbarIndex(
 startIndex: number,
 delta: number,
 predicate: (slot: HotbarSlot, index: number) => boolean,
): number | null {
...
export function findNextOwnedSeedHotbarIndex(
 startIndex: number,
 delta: number,
 seedCount: (seedId: string) => number,
): number | null {
 return findNextHotbarIndex(
 startIndex,
 delta,
 (slot) => slot.kind === 'seed' && (slot.seedId ? seedCount(slot.seedId) > 0 : false),
 );
}

export function ownedSeedHotbarIndex(
 seedId: string,
 seedCount: (seedId: string) => number,
): number | null {
...
export function hotbarSlotDisplayName(
 slot: HotbarSlot,
 resolveSeedName: (seedId: string) => string,
 resolveActionName: (kind: Exclude<HotbarSlotKind, 'seed'>) => string,
): string {
 if (slot.kind === 'seed') return resolveSeedName(slot.seedId ?? '');
 return resolveActionName(slot.kind);
}

export function hotbarStatusText(
 index: number,
 resolveSeedName: (seedId: string) => string,
 resolveActionName: (kind: Exclude<HotbarSlotKind, 'seed'>) => string,
): string {
...
export function hotbarToastPresentation(
 index: number,
 resolveSeedName: (seedId: string) => string,
 resolveActionName: (kind: Exclude<HotbarSlotKind, 'seed'>) => string,
