...
export type OverlayToastKind = 'exit-location-selection' | 'exit-interaction-panel' | 'pause' | 'resume' | 'open-inventory' | 'close-inventory';
...
export function deriveFarmActionOutcome(kind: FarmActionFeedbackKind, before: readonly FarmTileSnapshot[], after: readonly FarmTileSnapshot[]): FarmActionOutcome {
...
function didFarmActionAffectTile(kind: FarmActionFeedbackKind, before: FarmTileSnapshot, after: FarmTileSnapshot): boolean {
  switch (kind) {
    case 'till':
...
export function farmActionBlockedToastPresentation(kind: FarmActionFeedbackKind, reason: FarmActionBlockedReason | null, options?: FarmActionFeedbackOptions): FarmActionToastPresentation {
  return {
...
export function sowUnavailableToastPresentation(options?: SowUnavailableToastOptions): FarmActionToastPresentation {
  const normalizedRootAssetId = options?.assetIdOverride ? normalizeFarmsteadRootAssetId(options.assetIdOverride) : undefined;
  return {
    message: '无可播种种子，先去集市补货续上第二轮药材',
    assetId: options?.seedId ? (itemIconAssetId(options.seedId) ?? normalizedRootAssetId ?? 'loc.farmstead') : (normalizedRootAssetId ?? 'loc.valley-market')
  };
}
...
export function cultivationPanelToastPresentation(visible: boolean, assetIdOverride?: string): CultivationPanelToastPresentation {
  return {
...
export function overlayToastPresentation(kind: OverlayToastKind, assetIdOverride?: string): OverlayToastPresentation {
