...
function formatFindings(findings) {
  ...
  return findings
    .slice(0, 40)
    .map(finding => ` - ${finding.severity} ${finding.kind} ${finding.reviewStatus}: ${finding.path}:${finding.line} ${finding.match}`)
    .join('\n');
}

function formatReport(report) {
  return ['Public content audit', '', 'Content findings:', ` files-scanned: ${report.counts.filesScanned}`, ...severityOrder.map(severity => ` ${severity}: ${report.counts[severity]}`), ` actionable: ${report.counts.actionable}`, ` reviewed-guardrail: ${report.counts.reviewedGuardrail}`, ...findingKinds.map(kind => ` ${kind}: ${report.counts.byKind[kind]}`), '', 'Findings:', formatFindings(report.findings), '', 'High-risk findings fail the portfolio preflight; actionable info findings need release review, while reviewed-guardrail findings are expected self-check references in public governance tests/tools.'].join('\n');
}
