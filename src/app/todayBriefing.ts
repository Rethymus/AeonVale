...
function onboardingObjectiveAssetId(state: GameState, ctx: SimContext, objectiveId: OnboardingObjectiveId | null): string | undefined {
 ...
 case 'first-sow':
 return firstOwnedSeedAssetId(state, ctx)
	?? itemIconAssetId('seed.mossling', ctx.content)
 ?? locationPreviewAssetId('herb-plot');
 case 'first-water':
	return itemIconAssetId('item.water-pail', ctx.content)
 ?? locationPreviewAssetId('herb-plot');
 case 'first-harvest':
...
function formatUpcoming(entry: CalendarEntry): string {
 const prefix = (entry.daysFromNow ?? 0) <= 0
 ? '今日'
 : (entry.daysFromNow ?? 0) === 1
 ? '明日'
 : `${entry.daysFromNow}日后`;
...
function commissionReminderAssetId(itemId: string, npcId: string, ctx: SimContext): string | undefined {
 ...
 return locationPreviewAssetId(itemId === 'herb.mossling' ? 'creek-field' : 'herb-plot')
	?? itemIconAssetId(itemId, ctx.content);
 }
 if (npcId === 'npc.array-smith') {
	return locationPreviewAssetId('ruin-gate') ?? itemIconAssetId(itemId, ctx.content);
 }
 if (npcId === 'npc.wandering-cultivator') {
 return locationPreviewAssetId(itemId === 'item.beast-core' ? 'spirit-vein' : 'valley-market')
	?? itemIconAssetId(itemId, ctx.content);
 }
...
const goal = getPrimaryStayingWorldGoal(state);
 ...
 ? '主线：已满足条件，回人物面板领取本次主线奖励。'
 : `推进：${mainline.objective}`;
 }
...
function isPortfolioWelcomeBriefing(state: GameState, objectiveId: OnboardingObjectiveId | null, objective: string): boolean {
 return objectiveId === 'first-till'
 && objective === onboardingObjectiveHeadline('first-till')
 && state.day === 1
 && !state.player.flags.has(FIRST_HARVEST_FLAG)
 && !state.player.flags.has(FIRST_SHIPMENT_FLAG)
 && !state.player.flags.has(FIRST_SHIPPING_SETTLEMENT_FLAG)
 && !state.player.flags.has(FIRST_MARKET_RESTOCK_FLAG)
 && !state.player.flags.has(FIRST_SECOND_WATER_FLAG);
}
...
function onboardingObjectiveBriefingLines(
 state: GameState,
 ctx: SimContext,
 objectiveId: OnboardingObjectiveId | null,
 objective: string,
): string[] {
...
export function buildTodayBriefing(state: GameState, ctx: SimContext, objectiveText: string): TodayBriefingCard {
 ...
 const objectiveLines = objective.length > 0
 ? onboardingObjectiveBriefingLines(state, ctx, objectiveId, objective)
 : [];
 const headline = objective.length > 0
 ? `目标：${objective}`
 : reminderLine(state, ctx);
 const secondLine = objective.length > 0
 ? ''
 : farmFocusLine(state);
 const followUp = objective.length > 0
 ? ''
 : (() => {
 ...
 const fourthLine = objective.length > 0
 ? ''
 : (() => {
...
return {
 ...
 assetId: objective.length > 0
 ? (isPortfolioWelcomeBriefing(state, objectiveId, objective || canonicalObjective)
 ? 'logo.full'
 : objectiveAssetFallback(state, ctx, objectiveId))
 : nonObjectiveBriefingAssetId(state, ctx),
 };
}
