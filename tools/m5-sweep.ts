...
for (const c of CONFIGS) {
  ...
  const ok = inBand(n.asc, 0.3, 0.45) && inBand(v.asc, 0.75, 0.85);
  console.log(c.name.padEnd(16) + `${n.asc.toFixed(3)}/${n.death.toFixed(3)}`.padStart(20) + `${v.asc.toFixed(3)}/${v.death.toFixed(3)}`.padStart(20) + (ok ? ' ✅ HIT' : ''));
}
