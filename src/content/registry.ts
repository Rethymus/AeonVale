 ...
 */
import type { ContentRegistry, ItemDef, SpiritHerbDef } from './defs';
...
export function buildRegistry(): ContentRegistry {
  ...
  // schemaHash：内容指纹（docs/11 §3.2）。M1 用简化哈希。
  const schemaHash = simpleHash([...herbs.keys()].join(',') + '|' + [...items.keys()].join(','));
  return { herbs, items, seedToHerb, schemaHash };
}
