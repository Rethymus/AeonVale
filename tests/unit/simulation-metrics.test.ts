...
it('accepts an in-band point estimate as provisional M5 qualification', () => {
 ...
 const metrics = {
 bot: 'normal', runs: 500, ascension: estimate, death: wilsonInterval(313, 500), timeout: wilsonInterval(0, 500),
 duration: summarizeDuration(Array.from({ length: 500 }, (_, seed) => outcome({ seed })), 20),
 purpleEncounterCount: 0, purpleDeadlockCount: 0, purpleDeadlockSeeds: [], syntheticAssistanceRunRate: 1,
 meanSyntheticInterventions: 1, deterministic: true,
 };
...
it('keeps strict Wilson certification distinguishable from provisional qualification', () => {
 ...
 const metrics = {
 bot: 'normal', runs: 1_000, ascension: estimate, death: wilsonInterval(810, 1_000), timeout: wilsonInterval(0, 1_000),
 duration: summarizeDuration(Array.from({ length: 1_000 }, (_, seed) => outcome({ seed })), 20),
 purpleEncounterCount: 0, purpleDeadlockCount: 0, purpleDeadlockSeeds: [], syntheticAssistanceRunRate: 1,
 meanSyntheticInterventions: 1, deterministic: true,
 };
