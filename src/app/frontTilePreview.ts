...
const assetId = needsWater
 ? 'icon.item.water-pail'
 : needsQi
 ? 'icon.item.array-core'
 : hasSeed
 ? 'icon.seed.mossling'
 : 'loc.valley-market';

const actionLine = needsWater
 ? '先补水，再播种或补肥'
 : needsQi
 ? '先补灵，再播种或补肥'
 : hasSeed
...
function describeCrop(state: GameState, tile: Tile, crop: CropInstance, content: ContentRegistry): FrontTilePreview {
 ...
 const actionable = crop.stage === 'mature'
 ? '现在可收获'
 : crop.stage === 'withered'
 ? '已枯萎，收掉后再补种'
 : `湿润：${moistureStatus(tile)}｜灵气：${qiStatus(tile)}｜${protectedLine(state, tile.id)}`;

const assetId = crop.stage === 'mature'
 ? itemIconAssetId(crop.defId, content)
 : crop.stage === 'withered'
 ? itemIconAssetId(crop.defId, content)
 : !tile.wateredToday && tile.moisture < 55_000
 ? 'icon.item.water-pail'
 : !tile.channeledToday && tile.qiDensity < 55_000
 ? 'icon.item.array-core'
 : itemIconAssetId(crop.defId, content);
...
const status = facility.job == null
 ? '设施空闲，可立即投入使用'
 : facility.job.daysRemaining <= 0
 ? '产物已完成，可立即收取'
 : `加工中，还需 ${facility.job.daysRemaining} 日`;
 const jobLine = facility.job == null
 ? ''
 : facilityJobLine(facility.job.inputItemId, facility.job.outputItemId, content);
 return {
...
const def = content.arrays.get(array.defId);
 ...
 const status = array.active
 ? array.power > 0
 ? `阵势运转中｜覆盖 ${array.coverageTileIds.length} 格｜半径 ${radius}`
 : '阵势尚在，但灵力已尽'
 : '阵势未激活';
 const nextStep = def?.needsMetalCore
 ? '以金属性灵草为阵眼，外围可作引兽避雷田'
 : '护住核心药草区，减少雷击与失养风险';
