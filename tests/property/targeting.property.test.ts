...
describe('PBT-05: 修为单调 — 淬体后 cultivation ≥ 之前', () => {
  it('单次 runTribulation 存活后：cultivation 不减（淬体为正增益）', () => {
    fc.assert(
      fc.property(fc.integer({ min: 1, max: 9999 }), fc.integer({ min: 1, max: 5 }), fc.integer({ min: 1, max: 6 }), (seed, boltCount, stage) => {
  ...
  it('多次天劫序列：cultivation 严格递增（blastRadius=100 确保命中）', () => {
    fc.assert(
      fc.property(fc.integer({ min: 1, max: 500 }), fc.integer({ min: 2, max: 4 }), (seed, bolts) => {
    ...
    fc.assert(
      fc.property(fc.integer({ min: 1, max: 999 }), seed => {
        ...
        runTribulation(
          ...
          {
            stage: 1,
            boltCount: 0, // 零雷 → 无淬体 → cultivation 不变
            policy: { blockChance: 0 },
            blastRadius: 100
          },
          ctx
