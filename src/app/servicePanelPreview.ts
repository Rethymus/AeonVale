...
export function quickServiceUnavailableToastPresentation(kind: 'staying-commission' | 'tea-shed' | 'greenhouse', stayingWorldOnly = false, state?: GameState): ServiceToastPresentation {
  switch (kind) {
    case 'staying-commission':
      if (state?.activeEvent) {
        return {
          message: stayingWorldOnly ? '镇守告示今日随节停摆｜先去会场看当期事务' : '公告板今日随节停市｜先去会场看当期事务',
...
export function festivalResultToastPresentation(rewards: FestivalRewardLike[], content: ContentRegistry): ServiceToastPresentation {
  const names = rewards.map(reward => `${content.items.get(reward.itemId)?.displayName ?? reward.itemId}×${reward.count}`).join('、');
...
export function festivalPanelPreview(state: GameState): ServicePanelPreview {
  ...
  const actionLine = stallGoods.length > 0 ? '可先逛摊补节货，再顺手参与会场试礼' : '先去会场探风声，别把今日补货指望压在这里';
...
export function teaShedResultToastPresentation(outcome: 'success' | 'failure', resultOrReason: TeaShedVisitResultLike | string): ServiceToastPresentation {
  if (outcome === 'failure') {
    return {
...
export function greenhouseResultToastPresentation(outcome: 'success' | 'failure', resultOrReason: GreenhouseVisitResultLike | string, content: ContentRegistry): ServiceToastPresentation {
  if (outcome === 'failure') {
    return {
  ...
  const nurseryText = result.nurseryTier > 0 ? `，苗床${result.nurseryTier}阶·槽位${result.nurseryCapacity - result.nurserySlotsRemaining}/${result.nurseryCapacity}` : '';
