...
export function certifyM5Target(metrics: M5CohortMetrics, minRate: number, maxRate: number, minimumSample = 500): CertificationResult {
  if (metrics.runs < minimumSample) return { status: 'insufficient-sample', failures: [`${metrics.bot}: ${metrics.runs}/${minimumSample} samples`] };
  ...
  if (failures.length) return { status: 'regressed', failures };
  const intervalInside = metrics.ascension.wilsonLow95 >= minRate && metrics.ascension.wilsonHigh95 <= maxRate;
  return { status: intervalInside ? 'certified' : 'provisional', failures: [] };
}
