...
export function storagePanelPreview(mode: 'deposit' | 'withdraw', choice: StoragePreviewChoice, used: number, capacity: number, content: ContentRegistry): LogisticsPreview {
  const statusLine = mode === 'deposit' ? '高频材料入仓，给采收、补种、炼丹与阵材周转腾位' : '取回要加工、炼丹、布阵或出货的物资';
  return {
    title: itemName(choice.itemId, content),
    details: [mode === 'deposit' ? '存入仓库' : '取出仓库', `数量 × ${choice.count}${qualityLine(choice.quality)}`, `仓库占用 ${used}/${capacity}`, statusLine].join('\n'),
...
export function shippingPanelPreview(mode: 'normal' | 'quality', choice: ShippingPreviewChoice, content: ContentRegistry): LogisticsPreview {
  const total = choice.unitPrice * choice.count;
  const statusLine = mode === 'normal' ? '回笼灵石，补种、炉料与备劫消耗不断档' : '兑现高品质收成，换更稳的丹药、阵材与备劫余量';
  return {
    title: itemName(choice.itemId, content),
    details: [mode === 'normal' ? '普通出货' : '品质出货', mode === 'quality' ? `${QUALITY_LABEL[choice.quality ?? 'mortal']} × ${choice.count}` : `数量 × ${choice.count}`, `单价 灵石 × ${choice.unitPrice}｜本次 × ${total}`, statusLine].join('\n'),
    assetId: itemIconAssetId(choice.itemId, content),
