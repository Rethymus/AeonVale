...
export function buildResultToastPresentation(
 kind: FacilityKind,
 outcome: 'success' | 'failure',
 detail?: string,
): FacilityToastPresentation {
 return {
 message: outcome === 'success'
 ? `建造完成：${FACILITY_LABEL[kind]}${detail ? `｜${detail}` : ''}`
 : `建造失败：${detail ?? FACILITY_LABEL[kind]}`,
 assetId: facilityAssetId(kind),
 };
}

export function upgradeResultToastPresentation(
 upgrade: UpgradeDef,
 outcome: 'success' | 'failure',
 ...
 return {
 message: outcome === 'success'
 ? `${upgrade.displayName}完成${detail ? `｜${detail}` : ''}`
 : `升级失败：${detail ?? upgrade.displayName}`,
 assetId: preview.assetId,
 };
}

export function buildToastPresentation(
 kind: FacilityKind,
 indexLabel: string,
 confirmHint: string,
 content: ContentRegistry,
...
export function upgradeToastPresentation(
 upgrade: UpgradeDef,
 indexLabel: string,
 confirmHint: string,
 content: ContentRegistry,
...
export function facilityCollectToastPresentation(
 choice: FacilityCollectPreviewChoice,
 indexLabel: string,
 confirmHint: string,
): FacilityToastPresentation {
 return {
