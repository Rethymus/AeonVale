...
const wardingDone = hasCommissionCompletionForDay(state, evaluatedDay)
 || hasResolvedStayingWorldIncidentForDay(state, evaluatedDay)
 || state.events.some((event) => event.type === 'special-order-progress' || event.type === 'special-order-complete');
...
const beforeGreenhouseClimate = staying.greenhouseClimate;
 ...
 const climateGain = 6 * MILLI
 + Math.min(6 * MILLI, staying.greenhouseCareStreak * MILLI)
 + greenhouseClimateCareGainBonus(state)
 + cultivationBalance.diversityBonus
 - cultivationBalance.monoculturePenalty
 + (insulationClimateBonus.careGainBonus + wardenClimateBoost.careGainBonus) * MILLI;
 staying.greenhouseClimate = clampGreenhouseClimate(staying.greenhouseClimate + climateGain);
 } else {
 staying.greenhouseCareStreak = 0;
 const climateLoss = Math.max(
 MILLI,
 5 * MILLI
 + Math.min(9 * MILLI, staying.neglectedQuietDays * MILLI)
 - greenhouseClimateNeglectBuffer(state)
 - (insulationClimateBonus.neglectBuffer + wardenClimateBoost.neglectBuffer) * MILLI,
 );
