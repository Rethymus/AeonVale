...
export function hasWardPillReady(state: GameState): boolean {
 return state.player.wardMitigation > 0
 || carriedItemCount(state, 'pill.ward-basic') > 0
 || carriedItemCount(state, 'pill.ward-greater') > 0
 || carriedItemCount(state, 'pill.ward-heaven') > 0;
}
